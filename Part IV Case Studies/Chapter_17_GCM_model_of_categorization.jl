...
# ╔═╡ 60829413-cbeb-482c-ba89-2e18aa726cbd
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ a39db258-3426-11ee-3466-b1cbf9e23784
md"# 17. The GCM model of categorization"

# ╔═╡ 535dff8e-d634-4ea9-8a02-875f7d306fb2
md"## 17.1. The GCM model"

# ╔═╡ 98051ba8-62d8-4fe7-91a1-1019f251c2e9
@model function m17_1(y, t, d1, d2, a)
	c ~ Uniform(0, 5)
	w ~ Uniform(0, 1)
	b = 0.5
	s = exp.(-c * (w * d1 + (1 - w) * d2))
	Sum_as = sum(a .* s, dims = 2)
	Sum_mas = sum((1 .- a) .* s, dims = 2)
	r = (b * Sum_as) / (b * Sum_as + (1 - b) * Sum_mas)
	y .~ Binomial.(t, r)
end;

# ╔═╡ e980071b-7e57-437c-8421-d20439fbf928
#  Data
begin
	data = matopen("/Users/semih/Desktop/KruschkeData.mat");
	nstim = 8
	nsubj = 40
	t = nstim * nsubj
	a =  read(data, "a")[1, :]
	...
	d2 = read(data, "d2")
	x = read(data, "x")
	a1 = reshape(collect(Iterators.flatten(fill(2 .- a, nstim))), nstim, nstim)'
end;

# ╔═╡ e1be829f-f6c3-42d8-9d49-00925568a6bd
chns_m17_1 = sample(m17_1(y, t, d1, d2, a1), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 741a4ea2-da9b-4f24-b0b9-6eaabdbeec5c
describe(chns_m17_1)

# ╔═╡ e90eba5a-00c0-42a3-957a-72518ce7409b
plot(chns_m17_1)

# ╔═╡ e5b7a2e6-c452-4469-a194-7981e786ea5a
begin
	post_c = chns_m17_1["c"][:];
	post_w = chns_m17_1["w"][:];
end;

# ╔═╡ ab3bf3f5-a11c-4259-beac-6f16b71a0d30
scatter(post_c, post_w, alpha = 0.5, xlabel = "Generalization",
	ylabel = "Attention Weight", xlims = [0, 5], ylims = [0, 1], 
	label = false)

# ╔═╡ 8cabd734-a4bc-43f0-8447-51d8184071ca
# Posterior predictions
begin
	ypred = Matrix{Int64}(undef, 4000, 8)
	b = 0.5
	for i in 1:4000
		s = exp.(-post_c[i] * (post_w[i] * d1 + (1 - post_w[i]) * d2))
		Sum_as = sum(a1 .* s, dims = 2)
		Sum_mas = sum((1 .- a1) .* s, dims = 2)
		r = (b * Sum_as) / (b * Sum_as + (1 - b) * Sum_mas)
		yp = rand.(Binomial.(t, r))
		
		sumy = Vector{Int64}(undef, 8)
		for j in 1:8
			sumy[j] = sum(yp[j, :])
		end;
		ypred[i, :] = sumy
...
# ╔═╡ f3084176-6002-4d6c-9471-f5c06459e0e0
begin
	violin(ypred, xticks = 1:8, yticks = ([0, t], [i for i in ["B", "A"]]),
		ylims = [0, t], xlabel = "Stimulus", ylabel = "Category Decision",
		color = "gray", alpha = 0.5)
	errorline!(1:8, ypred, errorstyle = :plume, legend = false)
end

# ╔═╡ 78d46cb6-a2ab-4c78-9518-dbf824127bdd
md"## 17.2. Individual differences in the GCM"

# ╔═╡ 413e4491-df24-4332-86af-3d4dcbf9bd3e
@model function m17_2(y, d1, d2, a)
	c ~ filldist(Uniform(0, 5), 40)
	w ~ filldist(Uniform(0, 1), 40)
	b = 0.5

	# Similarities
	s = Array{Float64}(undef, 8, 40, 8)
	for i in 1:8
		for j in 1:8
			for k in 1:40
	...
	numerator = Array{Float64}(undef, 8, 40, 8)
	denominator = Array{Float64}(undef, 8, 40, 8)
	for i in 1:8
		for j in 1:8
			for k in 1:40
				numerator[i, k, j] = (a[j] == 1) * b * s[i, k, j]
				denominator[i, k, j] = (a[j] == 1) * b * s[i, k, j] +
				(a == 2) * (1 - b) * s[i, k, j]
			end;
		end;
	end;
	...
	for i in 1:8
    	for k in 1:40
			y[k, i] ~ Binomial(8, r[i, k])
		end;
	end;
end;

# ╔═╡ 5d6b96ac-fb5e-4253-b6a0-57c4749c89f6
chns_m17_2 = sample(m17_2(x, d1, d2, a), MH(), MCMCThreads(), 10000, 4);

# ╔═╡ 5ac4dd10-9f66-4897-b9fd-dd5879077a47
describe(chns_m17_2)

# ╔═╡ 648ab809-467e-4668-aee5-b1579352698a
plot(1:8, [x[i, :] for i in 1:40], ylims = [-0.25, 8.25], layout = (5, 8),
	label = false, xticks = false, yticks = false)

# ╔═╡ 6245672f-0170-4057-bf8b-0fc5c07a5cea
begin
	post_c2 = Vector{Float64}(undef, 40)
	post_w2 = Vector{Float64}(undef, 40)
	for i in 1:40
		post_c2[i] = mean(chns_m17_2["c[$i]"][:]);
		post_w2[i] = mean(chns_m17_2["w[$i]"][:]);
	end;
end;

# ╔═╡ 2582bb9a-78ad-44fa-bf1b-f3fa73714eff
scatter(post_c2, post_w2, alpha = 0.5, xlabel = "Generalization",
	ylabel = "Attention Weight", xlims = [0, 4], ylims = [0, 1], 
	label = false)

# ╔═╡ 3d3682c1-b426-4e90-bbe8-263ad4b5ea2a
md"## 17.3. Latent groups in the GCM"

# ╔═╡ f73efda4-ea46-4fad-b584-919b2ac01817
@model function m17_3(y, d1, d2, a)
	# Priors
	b = 0.5
	...
	phic ~ Beta(1, 1)
	phig ~ Beta(1,1)
	zc ~ filldist(Bernoulli(phic), 40)
 	zg ~ filldist(Bernoulli(phig), 40)
  	zg1 = zg .+ 1
  	z = (zc .== 0) .* zg1 + 3 * (zc .== 1)
	...
	wpredg = Vector{Float64}(undef, 2)
	for i in 1:2
  		wpredg[i] ~ truncated(Normal(muw[i], lambdaw), 0, 1)
	end;
  	cpredg ~ filldist(truncated(Normal(muc, lambdac), 0, Inf), 2)

	# Subject Parameters
  	c ~ filldist(truncated(Normal(muc, lambdac), 0, Inf), 40)

	w = Vector{Float64}(undef, 40)
	for i in 1:40
		ind = zg1[i]
  		w[i] ~ truncated(Normal(muw[ind], lambdaw), 0, 1)
	end;

	# Similarities
	s = Array{Float64}(undef, 8, 40, 8)
	spredg = Array{Float64}(undef, 2, 8, 8)
	for i in 1:8
		for j in 1:8
			for k in 1:40
				s[i, k, j] = exp(-c[k] * (w[k] * d1[i, j] + (1 - w[k]) * d2[i, j]))
			...
			for g in 1:2
      			spredg[g, i, j] = exp(-cpredg[g] * (wpredg[g] * d1[i, j] +
				(1 - wpredg[g]) * d2[i, j]))
			end;
		end;
	end;
	...
	numeratorpredg = Array{Float64}(undef, 2, 8, 8)
	denominatorpredg = Array{Float64}(undef, 2, 8, 8)
	for i in 1:8
		for j in 1:8
			for k in 1:40
				numerator[i, k, j] = (a[j] == 1) * b * s[i, k, j]
			...
			for g in 1:2
				numeratorpredg[g, i, j] = (a[j] == 1) * b * spredg[g, i, j]
      			denominatorpredg[g, i, j] = (a[j] == 1) * b * spredg[g, i, j]
			end;
		end;
	end;
	...
	# Base Decision Probabilities
	r = Matrix{Float64}(undef, 8, 40)
	rpredg = Matrix{Float64}(undef, 3, 40)
	for i in 1:8
    	for k in 1:40
      		r[i, k] = (zc[k] == 0) * sum(numerator[i, k, :]) /
			sum(denominator[i, k, :])
			for g in 1:2
				rpredg[g, i] = sum(numeratorpredg[g, i, :]) /
				sum(denominatorpredg[g, i, :])
			end;
			rpredg[3, i] = 0.5
		end;
	end;

	predyg = Matrix{Float64}(undef, 3, 8)
	# Decision Data
	for i in 1:8
		# Groups
		for g in 1:3
    		predyg[g, i] ~ Binomial(8, rpredg[g, i])
		end;
...
# ╔═╡ 6f7d22d8-725f-4ad8-a29b-96b388f5e252
chns_m17_3 = sample(m17_3(x, d1, d2, a), MH(), MCMCThreads(), 1000, 4);
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "69c48a6529341edf0674b51064dfad5e0a416c30"
