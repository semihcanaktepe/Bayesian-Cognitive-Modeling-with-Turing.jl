...
# ╔═╡ 716e3427-70c9-417a-96b4-72c5ff29b5fc
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 14b8f74e-3365-11ee-3293-adfa520be4b9
# 15. The SIMPLE model of memory

# ╔═╡ 2934f662-36cb-42cd-a4b8-6915972585fd
# 15.1. The SIMPLE model

# ╔═╡ 884237f0-a9b3-474a-8e09-79ade69ee492
@model function m15_1(y, n, dsets, listlength, m)
	# Priors
	c ~ filldist(Uniform(0, 100), dsets)
	s ~ filldist(Uniform(0, 100), dsets)
	t ~ filldist(Uniform(0, 1), dsets)
	# Three-dimentional arrays of sim, disc, and resp
	eta = Array{Float64}(undef, 40, 40, 6)
	d = Array{Float64}(undef, 40, 40, 6)
	r = Array{Float64}(undef, 40, 40, 6)
	# Theta matrix
	theta = Matrix{Float64}(undef, 40, 6)
	for x in 1:dsets
    	for i in 1:listlength[x]
      		for j in 1:listlength[x]
				# Similarities
				eta[i, j, x] = exp(-c[x] * abs(log(m[i, x]) - log(m[j, x])))
				# Discriminabilities
        		d[i, j, x] = eta[i, j, x] / sum(eta[i, 1:listlength[x], x])
        		# Response Probabilities
        		r[i, j, x] = 1 / (1 + exp(-s[x] * (d[i, j, x] - t[x])))
			end;
			theta[i, x] = min(1, sum(r[i, 1:listlength[x], x]))
		end;
	end;
	y .~ Binomial.(n, theta)
end;

# ╔═╡ a5a9182e-5460-4084-9eba-1d7187290a48
begin
	y = CSV.read("/Users/semih/Desktop/k_M.txt", header = false, DataFrame);
	m = CSV.read("/Users/semih/Desktop/m.txt", header = false, DataFrame);
	n = [1440, 1280, 1520, 1520, 1200, 1280];
	listlength = [10, 15, 20, 20, 30, 40];
	dsets = 6;
	ymat = Matrix(y)'
	mmat = Matrix(m)'
end;

# ╔═╡ d9fd34f0-c332-4110-a1b8-6d82a96acbd0
chns_m15_1 = sample(m15_1(ymat, n, dsets, listlength, mmat), 
	NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ cafe1f37-dbca-4e80-b88b-912722151c11
# 15.2. A hierarchical extension of SIMPLE

# ╔═╡ ee1069a9-825c-4b17-a100-0a61f89c0b58
@model function m15_2(y, n, dsets, gsets, listlength, m)
	# Priors
	c ~ Uniform(0, 100)
	s ~ Uniform(0, 100)
	a1 ~ Uniform(-1, 0)
	a2 ~ Uniform(0, 1)
	t = Vector{Float64}(undef, gsets)
	# Three-dimentional arrays of sim, disc, and resp
	eta = Array{Float64}(undef, 40, 40, 6)
	d = Array{Float64}(undef, 40, 40, 6)
	r = Array{Float64}(undef, 40, 40, 6)
	...
	for x in 1:gsets
		# W = listlength
		t[x] = max(0, min(1, a1 * listlength[x] + a2))
    	for i in 1:listlength[x]
      		for j in 1:listlength[x]
				# Similarities
				eta[i, j, x] = exp(-c[x] * abs(log(m[i, x]) - log(m[j, x])))
...
# ╔═╡ 2d4c4497-2eae-461a-84bc-1d34e41c7126
# Data
gsets = 9;

# ╔═╡ 6e4b2e21-46c5-40ff-a5f4-bfa710705108
chns_m15_2 = sample(m15_2(ymat, n, dsets, gsets, listlength, mmat), 
	NUTS(), MCMCThreads(), 1000, 4);
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "f6fb19ddfe47b451f293269a148a871e6e79b742"
