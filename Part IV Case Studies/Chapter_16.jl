...
begin
	...
	cash = (data.cash .!= 0) .+ 0
	npumps = data[!, "pumps"];
	# Enumerate the pumps in each trial
	#ktemp = [repeat(1:npumps[i], npumps[i]) for i in 1:length(npumps)];
	ktemp = [1:npumps[i] for i in 1:length(npumps)];
	k = collect(Iterators.flatten(ktemp));
	# Whether cash taken at the given pump of a trial 
	dtemp = [cash[i] == 0 ? fill(cash[i], npumps[i]) : vcat(fill(0, npumps[i] - 1), 1) for i in 1:length(npumps)];
	d = collect(Iterators.flatten(dtemp));
end;

# ╔═╡ fae3b10b-e6c8-403e-91a8-18955ee745c8
chns_m16_1 = sample(m16_1(d, k, p), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 5bdccf71-3b4d-48cb-8b83-7c63cdd3cf77
describe(chns_m16_1)

# ╔═╡ 2ee50795-0325-42b5-939b-f18b6342f1cf
plot(chns_m16_1)
...
@model function m16_2(d, k, p, cond)
	mug ~ Normal(0, 10)
	mub ~ Normal(0, 10)
	sigmag ~ Normal(0, 10)
	sigmab ~ Normal(0, 10)
	lambdag = 1 / sigmag^2
	lambdab = 1 / sigmab^2
	gplus ~ filldist(Truncated(Normal(mug, lambdag), 0, Inf), length(unique(cond)))
	beta ~ filldist(Truncated(Normal(mub, lambdab), 0, Inf), length(unique(cond)))
	omega = -gplus[cond] ./ log(1 - p)
	theta = 1 ./ (1 .+ exp.(beta[cond] .* (k .- omega)))
	d .~ Bernoulli.(theta)
end;
...
# ╔═╡ 0ae51668-9156-4f77-895c-d8843f9ed031
begin
	# Cash or burst
	cash2 = (data2.cash .!= 0) .+ 0
	npumps2 = data2[!, "pumps"];
	# Enumerate the pumps in each trial
	ktemp2 = [1:npumps2[i] for i in 1:length(npumps2)];
	k2 = collect(Iterators.flatten(ktemp2));
	# Whether cash taken at the given pump of a trial 
	dtemp2 = [cash2[i] == 0 ? fill(cash2[i], npumps2[i]) : vcat(fill(0, npumps2[i] - 1), 1) for i in 1:length(npumps2)];
	d2 = collect(Iterators.flatten(dtemp2));
end;

# ╔═╡ 28bce095-0d06-4442-8a60-49bfb8258ca9
begin
	# Cash or burst
	cash3 = (data3.cash .!= 0) .+ 0
	npumps3 = data3[!, "pumps"];
	# Enumerate the pumps in each trial
	ktemp3 = [1:npumps3[i] for i in 1:length(npumps3)];
	k3 = collect(Iterators.flatten(ktemp3));
	# Whether cash taken at the given pump of a trial 
	dtemp3 = [cash3[i] == 0 ? fill(cash3[i], npumps3[i]) : vcat(fill(0, npumps3[i] - 1), 1) for i in 1:length(npumps3)];
	d3 = collect(Iterators.flatten(dtemp3));
end;

# ╔═╡ a57810a2-99fa-4975-87c2-1e657045bb6f
begin
	k1 = vcat(k, k2, k3);
	d1 = vcat(d, d2, d3);
	# Assign conditions
	cond1 = fill(1, length(k));
	cond2 = fill(2, length(k2));
	cond3 = fill(3, length(k3));
	cond = vcat(cond1, cond2, cond3);
end;

# ╔═╡ d18b3911-79bd-4ae1-9c71-208e5a9859eb
chns_m16_2 = sample(m16_2(d1, k1, p, cond), NUTS(), MCMCThreads(), 1000, 4);
