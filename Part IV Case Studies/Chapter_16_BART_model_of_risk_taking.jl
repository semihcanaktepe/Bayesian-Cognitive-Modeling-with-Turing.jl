...
# ╔═╡ 29ec97aa-58a2-4f19-a6f7-971b933eada6
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 1f83037a-3391-11ee-06bd-c901065a3871
md"# 16. The BART model of risk taking"

# ╔═╡ 6f5347bc-2da7-4c20-be09-44954b15d28e
md"## 16.1. The BART model"

# ╔═╡ f5b78f7f-9a60-43a2-9b8b-5a9a34837b30
@model function m16_1(d, options, p, ntrials)
	gplus ~ Uniform(0, 10)
	beta ~ Uniform(0, 10)
	omega = -gplus / log(1 − p)
	theta = Matrix{Float64}(undef, ntrials, 30)
	for j in 1:ntrials
		for k in 1:options[j]
			theta[j, k] = 1 - (1 / (1 + max(-15, min(15, exp(beta * (k - omega))))))
			d[j, k] ~ Bernoulli(theta[j, k])
		end;
	end;
end;

# ╔═╡ d1984a59-639d-4583-aff3-3e5ac469b2c9
# Data
data = CSV.read("/Users/semih/Desktop/GeorgeSober.txt", DataFrame);

# ╔═╡ 60ce9e22-9631-47bb-8d7b-3cd1b165b784
begin
	# Probability of the balloon bursting
	p = 0.15
	ntrials = 90
	# Cash or burst
	cash = (data.cash .!= 0) * 1
	npumps = data.pumps;
	options = cash .+ npumps;
	d = Matrix{Any}(missing, ntrials, 30);
	for j in 1:ntrials
		if npumps[j] > 0
			d[j, 1:npumps[j]] = fill(0, npumps[j])
		end;
		if cash[j] == 1
			d[j, (npumps[j] + 1)] = 1
		end;
...
# ╔═╡ fae3b10b-e6c8-403e-91a8-18955ee745c8
chns_m16_1 = sample(m16_1(d, options, p, ntrials), MH(), MCMCThreads(), 100000, 4);

# ╔═╡ 5bdccf71-3b4d-48cb-8b83-7c63cdd3cf77
describe(chns_m16_1)

# ╔═╡ 3fe66d5a-8f4e-4da7-90ce-a2d0c166b7f1
begin
	post_gplus = chns_m16_1["gplus"][:];
	post_beta = chns_m16_1["beta"][:];
end;

# ╔═╡ a3f9a33b-f362-4622-b282-ad5887ef11aa
begin
	p1 = histogram(npumps, bins = 7, xlabel = "Number of Pumps", ylabel = "Frequency",
	ylims = [0, 35], ytick = [0, 33], xticks = (1.5:7.5, [i for i in 1:7]),
		label = false)
	p2 = density(post_gplus, linewidth = 3, label = false, xlabel = "γ+",
		ylabel = "Posterior Density", xlims = [0.3, 1.2])
	p3 = density(post_beta, linewidth = 3, label = false, xlabel = "β",
		ylabel = "Posterior Density", xlims = [0.1, 1.5])
	plot(p1, p2, p3, layout = (1, 3))
end

# ╔═╡ 6cacdbdd-5c1d-476f-9179-7dc7c49f8d50
md"## 16.2. A hierarchical extension of the BART model"

# ╔═╡ b4f89694-f5c1-4a47-b086-1d4090a491ad
@model function m16_2(d, options, p, nconds, ntrials)
	mug ~ Normal(0, 10)
	mub ~ Normal(0, 10)
	sigmag ~ Normal(0, 10)
	sigmab ~ Normal(0, 10)
	lambdag = 1 / sigmag^2
	lambdab = 1 / sigmab^2
	gplus ~ filldist(Truncated(Normal(mug, lambdag), 0, Inf), nconds)
	beta ~ filldist(Truncated(Normal(mub, lambdab), 0, Inf), nconds)
	omega = -gplus / log(1 - p)
	theta = Array{Float64}(undef, nconds, ntrials, 30)
	for i in 1:nconds
		for j in 1:ntrials
			for k in 1:options[i, j]
				theta[i, j, k] = 1 - (1 / (1 + max(-15,
					min(15, exp(beta[i] * (k - omega[i]))))))
				d[i, j, k] ~ Bernoulli(theta[i, j, k])
			end;
		end;
	end;
end;

# ╔═╡ 05d8930f-9c29-4e23-9d6b-dc7efe95b24c
# Data
begin
	data2 = CSV.read("/Users/semih/Desktop/GeorgeTipsy.txt", DataFrame);
	data3 = CSV.read("/Users/semih/Desktop/GeorgeDrunk.txt", DataFrame);
end;

# ╔═╡ 62b02af1-016b-4b97-ae80-ea4d403cdd75
begin
	ds = [data, data2, data3];
	nconds = 3;
	cash2 = Matrix{Any}(undef, nconds, ntrials);
	npumps2 = Matrix{Any}(undef, nconds, ntrials);
	d2 = Array{Any}(undef, nconds, ntrials, 30);
	for i in 1:nconds
		cash2[i, :] = (ds[i].cash .!= 0) * 1
		npumps2[i, :] = ds[i].pumps
		for j in 1:ntrials
			if npumps2[i, j] > 0
				d2[i, j, 1:npumps2[i, j]] = fill(0, npumps2[i, j])
			end;
			if cash2[i, j] == 1
				d2[i, j, (npumps2[i, j] + 1)] = 1
			end;
		end;
	end;
	options2 = cash2 .+ npumps2;
end;

# ╔═╡ d18b3911-79bd-4ae1-9c71-208e5a9859eb
chns_m16_2 = sample(m16_2(d2, options2, p, nconds, ntrials),
	MH(), MCMCThreads(), 100000, 4);

# ╔═╡ 98438bf1-0910-4afd-b90f-f3928b9498b7
describe(chns_m16_2)

# ╔═╡ 0997a517-da3d-4b68-aefd-2f3c41d6bb26
begin
	p12 = histogram(npumps2[1, :], bins = 9, xlabel = "Number of Pumps",
		ylabel = "Sober", ylims = [0, 35], ytick = [0, 33], xlims = [0, 10],
		xticks = (1.5:10.5, [i for i in 1:9]), label = false)
	p22 = histogram(npumps2[2, :], bins = 9, xlabel = "Number of Pumps",
		ylabel = "Tipsy", ylims = [0, 35], ytick = [0, 24], xlims = [0, 10],
		xticks = (1.5:10.5, [i for i in 1:9]), label = false)
	p32 = histogram(npumps2[3, :], bins = 9, xlabel = "Number of Pumps",
		ylabel = "Drunk", ylims = [0, 35], ytick = [0, 30], xlims = [0, 10],
		xticks = (1.5:10.5, [i for i in 1:9]), label = false)
