...
# ╔═╡ 0c74664e-e881-44dc-b6b1-6854255ada49
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ e49aae28-32eb-11ee-3f70-a54aa992893e
md"# 13. Extrasensory perception"

# ╔═╡ 72ce1dd6-f5a2-4b01-88c0-e408146442d2
md"## 13.1. Evidence for optional stopping"

# ╔═╡ 4b1ab4f1-b408-4a77-b6d4-946559779827
@model function m13_1(x)
	mu1 ~ Normal(0, 0.001)
	mu2 ~ Normal(0, 0.001)
	...
	T = [sigma1^2 r * sigma1 * sigma2; r * sigma1 * sigma2 sigma2^2]
...
# ╔═╡ 04b12684-671f-49a7-a5da-6d62291b0b38
# Data
begin
	N = [100, 150, 97, 99, 100, 150, 200, 100, 50];
	E = [0.25, 0.20, 0.25, 0.20, 0.22, 0.15, 0.09, 0.19, 0.42];
	x = hcat(N, E);
end;

# ╔═╡ 44d553c2-4604-4f16-a98b-27c05eb641f4
chns_m13_1 = sample(m13_1(x), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ e741ff13-be2b-4f80-884b-8de588c6032a
describe(chns_m13_1)

# ╔═╡ b03553ef-d42e-4e7c-b689-0938ae462485
plot(chns_m13_1)

# ╔═╡ 4fae9a39-0e66-4cb2-ad73-15988555b29c
begin
	post_r = chns_m13_1["r"][:];
	r_pdf = kde(post_r);
	x1 = range(-1, 1, length = 200)
	prior_pdf = Vector{Float64}(undef, 200);
	prior_pdf = ones(200) * 0.5
	posterior = pdf(r_pdf, 0);
	prior = 0.5;
	BF01 = posterior / prior;
end;

# ╔═╡ f8591273-c001-4e75-b2d8-74de70f2a290
begin
	p1 = scatter(N, E, xlabel = "Number of Subjects", ylabel = "Effect Size",
	xlims = [0, 210], ylims = [0, 0.5], label = false, xticks = range(0, 200, 5))
	p2 = plot(x1, pdf(r_pdf, x1), xlabel = "Correlation", 
		ylabel = "Density", label = "Posterior", linewidth = 3)
	plot!(x1, prior_pdf, linewidth = 3, label = "Prior", linestyle = :dot)
	plot!([0, 0], [posterior, prior], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior, prior], color = "black", label = "")
	plot(p1, p2, layout = (1, 2))
end

# ╔═╡ a0782fb8-c6b3-496f-a88b-0f2372e7f34f
md"## 13.2. Evidence for differences in ability"

# ╔═╡ cd184154-2e50-48ed-8b2a-5b71d6240b59
# Define probit function
function Phi(x::Real)
	return 0.5 + 0.5 * erf(x / sqrt(2))
end;

# ╔═╡ ea95d370-1ca4-4de3-828d-b45ffc96ee1e
@model function m13_2(k)
	...
	T = [sigma1^2 r * sigma1 * sigma2; r * sigma1 * sigma2 sigma2^2]
	TI = Hermitian(inv(T + 1e-9 * I))
	thetap ~ filldist(MvNormal([mu1, mu2], TI), size(k)[1])
	theta = Phi.(thetap')
	k .~ Binomial.(60, theta)
end;

# ╔═╡ 21bc0625-4bcc-43b0-9616-1d863e108a32
# Data
begin
	prc1er = [0.6000000, 0.5333333, 0.6000000, 0.6000000, 0.4666667, 
	...
	prc2er = [0.3333333, 0.6000000, 0.5333333, 0.2666667, 0.6666667,
	...
	k = round.(Int64, hcat(prc1er, prc2er)*60);
end;

# ╔═╡ a3b57a03-5bbd-4f52-b474-7b1083684f2a
# Results do not replicate.
chns_m13_2 = sample(m13_2(k), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 24318fc2-cb9a-4dbe-8c6d-cf69da581d97
describe(chns_m13_2)

# ╔═╡ 34727c08-96bd-444e-9311-45329df6b2c0
begin
	post_r2 = chns_m13_2["r"][:];
	r_pdf2 = kde(post_r2);
	posterior2 = pdf(r_pdf2, 0);
	BF01_2 = posterior2 / prior;
end;

# ╔═╡ 5830d682-b52f-44b4-beb0-364443ce5ac8
begin
	p12 = histogram2d(k[:,1], k[:,2], xlims = [0, 60], ylims = [0, 60], label = false,
		xlabel = "Performance Session 1", ylabel = "Performance Session 2",
		bins = (20, 20))
	p22 = plot(x1, pdf(r_pdf2, x1), xlabel = "Correlation", xlims = [-0.05, 1],
		ylabel = "Density", label = "Posterior", linewidth = 3)
	plot!(x1, prior_pdf, linewidth = 3, label = "Prior", linestyle = :dot)
	plot!([0, 0], [posterior2, prior], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior2, prior], color = "black", label = "")
	plot(p12, p22, layout = (1, 2))
end

# ╔═╡ f3c6a070-c7ac-4c86-8ddb-3b35a141381e
md"## 13.3. Evidence for the impact of extraversion"

# ╔═╡ 425087ff-3f6f-4d55-87de-6955be6d02df
@model function m13_3(k, x)
	# Default priors did not work
	mu1 ~ Normal(0, 1)
	mu2 ~ Normal(0, 1)
	lambda1 ~ Gamma(1, 1)
	...
	T = [sigma1^2 r * sigma1 * sigma2; r * sigma1 * sigma2 sigma2^2]
	TI = Hermitian(inv(T + 1e-9 * I))
	thetap ~ filldist(MvNormal([mu1, mu2], TI), length(k))
	theta1 = Phi.(thetap[1, :])
	theta2 = 100 * Phi.(thetap[2, :])
	k .~ Binomial.(60, theta1)
	x .~ Normal.(theta2, lambdax)
end;

# ╔═╡ 6e1afb37-3036-4853-9fa2-03c8533ca5a8
# Data
begin
	k2 = [36, 32, 36, 36, 28, 40, 40, 24, 36, 36, 28, 40, 28, 
	...
	x2 = [50, 80, 79, 56, 50, 80, 53, 84, 74, 67, 50, 45, 62, 
...
# ╔═╡ 1cbe504e-913a-4457-b021-50a922c311d6
# Does not sample, MH sampler works, but results do not replicate.
chns_m13_3 = sample(m13_3(k2, x2), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ e449579d-0bf0-46a2-b0f2-919bdbe2cd50
describe(chns_m13_3)

# ╔═╡ a17ec524-c6dc-4a8e-913f-7a85a5c697f5
begin
	post_r3 = chns_m13_3["r"][:];
	r_pdf3 = kde(post_r3);
	posterior3 = pdf(r_pdf3, 0);
	BF01_3 = posterior3 / prior;
end;

# ╔═╡ b7d700a6-83ae-45fa-bec6-90a543ae7248
begin
	p13 = scatter(x2, k2, xlabel = "Extraversion", ylabel = "Performance Session 1",
	xlims = [0, 100], ylims = [0, 60], label = false)
	p23 = plot(x1, pdf(r_pdf3, x1), xlabel = "Correlation", 
		ylabel = "Density", label = "Posterior", linewidth = 3)
	plot!(x1, prior_pdf, linewidth = 3, label = "Prior", linestyle = :dash)
	plot!([0, 0], [posterior3, prior], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior3, prior], color = "black", label = "")
	plot(p13, p23, layout = (1, 2))
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "3d59447ca459cb1cf865504a52698fcea1c661e9"
...
# ╠═4b1ab4f1-b408-4a77-b6d4-946559779827
# ╠═04b12684-671f-49a7-a5da-6d62291b0b38
# ╠═3aabdb55-eaf1-4835-832f-b312f4faaca2
# ╠═44d553c2-4604-4f16-a98b-27c05eb641f4
# ╠═e741ff13-be2b-4f80-884b-8de588c6032a
# ╠═b03553ef-d42e-4e7c-b689-0938ae462485
# ╠═4fae9a39-0e66-4cb2-ad73-15988555b29c
