...
# chns_m18_2 = sample(m18_2(y1, t_ttb, t_wadd), NUTS(), MCMCThreads(), 1000, 4);
...
@model function m18_3(y)
	...
	for i in 1:20
    	for q in 1:30
    		...
    		for j in 1:9
				s[i, j] = sortperm(stmp[i, 1:nc], dims = 1);
			end;
      		for j in 1:9
...
# chns_m18_3 = sample(m18_3(y1), NUTS(), MCMCThreads(), 1000, 4);
...
@model function m18_4(y, x, m, p)
	...
	for i in 1:30
		z[:, 1] ~ filldist(Bernoulli(phi[i]), 30)'
	end;
	z1 = reshape(z, 20, 30)
	gamma ~ Uniform(0.5, 1)
	tmp1 = Array{Int64}(undef, 20, 30, 9)
	...
	tmp4 = Array{Int64}(undef, 20, 30, 9);
	tmp5 = Matrix{Int64}(undef, 20, 30);
	tmp6 = Matrix{Int64}(undef, 20, 30);
	for i in 1:20
		for q in 1:30
			for j in 1:9
				tmp4[i, q, j] = (m[p[q, 1], j] - m[p[q, 2], j]) * x[j]
	...
	for i in 1:20
		for q in 1:30
			y[i, 1] ~ Bernoulli(dec[t[i, q, z1[i, q]]])
		end;
	end;
end;

# ╔═╡ 07eed386-0ce8-4a1b-9eed-98102551d0dc
# chns_m18_4 = sample(m18_4(y1, x, m, p), NUTS(), MCMCThreads(), 1000, 4);
