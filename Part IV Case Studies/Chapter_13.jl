...
@model function m13_2(k)
	...
	T = [1 / lambda1 r * sigma1 * sigma2; r * sigma1 * sigma2 1 / lambda2]
	TI = Hermitian(inv(T + 1e-9 * I))
	thetap = Matrix{Real}(undef, size(k))
	for i in 1:size(k)[1]
		thetap[i, :] ~ MvNormal([mu1, mu2], TI)
	end;
	theta = Phi.(thetap)
	k .~ Binomial.(60, theta)
end;
...
@model function m13_3(k, x)
	...
	T = [1 / lambda1 r * sigma1 * sigma2; r * sigma1 * sigma2 1 / lambda2]
	TI = Hermitian(inv(T + 1e-9 * I))
	thetap = Matrix{Real}(undef, 100, 2)
	for i in 1:length(k)
		thetap[i, :] ~ MvNormal([mu1, mu2], TI)
	end;
	theta1 = Phi.(thetap[:, 1])
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "0a648c5757d98617cefcc52daf8ab7e01eb0d544"
...
# ╠═425087ff-3f6f-4d55-87de-6955be6d02df
# ╠═6e1afb37-3036-4853-9fa2-03c8533ca5a8
# ╠═f579db1b-4f82-4e50-9773-7b985dcf8114
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-000000000002
