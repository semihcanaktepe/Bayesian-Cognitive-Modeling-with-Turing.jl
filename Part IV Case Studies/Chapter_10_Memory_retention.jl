...
# ╔═╡ 1ceb4d91-c7c3-445a-93c0-2307d6edabc4
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ f1b9d55c-31f1-11ee-3c79-11ce013766da
md"# 10. Memory retention"

# ╔═╡ 6e861680-7794-4209-91bd-25de40e1fe87
md"## 10.1. No individual differences"

# ╔═╡ 755620fa-6f7d-4d6e-9453-5f3b5d379de0
@model function m10_1(k, t, n)
	alpha ~ Beta(1, 1)
	beta ~ Beta(1, 1)
	theta = min.(1, exp.(-alpha * t) .+ beta)
	k .~ Binomial.(n, theta)
end;

# ╔═╡ afb8c024-a0a0-44c7-8f35-1bc0ff053b89
# Data
begin
	# time intervals
	t = [1, 2, 4, 7, 12, 21, 35, 59, 99];
	# scores
	k = reshape([18, 18, 16, 13, 9, 6, 4, 4, 4,
		17, 13,  9,  6, 4, 4, 4, 4, 4,
		14, 10,  6,  4, 4, 4, 4, 4, 4], 9, 3);
	# number of items
	n = 18;
end;

# ╔═╡ 3952a51c-7acb-4d39-b12d-bdc9c9a26a5e
chns_m10_1 = sample(m10_1(k, t, n), NUTS(0.99), MCMCThreads(), 1000, 4);

# ╔═╡ a90a61bc-37b9-4ea1-a51d-d82c90b94afe
describe(chns_m10_1)

# ╔═╡ fdc6c901-3d1f-41b3-9921-03d1aadba544
plot(chns_m10_1)

# ╔═╡ c97b494a-f61d-4f9e-be22-f4cde7215a35
begin
	post_alpha = chns_m10_1["alpha"][:];
	post_beta = chns_m10_1["beta"][:];
end;

# ╔═╡ 760680a4-004a-4359-a2c0-359ec3bbc020
marginalkde(post_alpha, post_beta, xlabel = "Decay Rate", ylabel = "Baseline",
	clip = ((-10, 5), (-10, 5)))

# ╔═╡ 48bdbb00-2cbc-4bdf-b5c4-dc3e202a8f75
begin
	t2 = [1, 2, 4, 7, 12, 21, 35, 59, 99, 200];
	post_k = Matrix{Int64}(undef, 4000, 10);
	for i in 1:4000
		for j in 1:10
			post_k[i, j] = rand(Binomial(18, exp.(-post_alpha[i] * t2[j]) + post_beta[i]));
		end;
	end;
end;

# ╔═╡ 92150799-5921-41cb-a8a9-961008a5f664
begin
	errorline(1:10, post_k, linewidth = 3, ylims = [0, 20], label = "Fitted k",
		title = "Posterior Plot", xlabel = "Time Lags", ylabel = "Retention Count",
		xticks = (1:10, [i for i in t2]))
	errorline!(1:9, k,  linewidth = 3, label = "Mean k")
	scatter!(1:9, k, label = ["Subject 1" "Subject 2" "Subject 3"])
end

# ╔═╡ 9491a6d7-0d3b-4be0-a7f5-e69094cd45bd
md"## 10.2. Full individual differences"

# ╔═╡ b9535e2c-1d21-4f00-81b9-69d112bbec7b
@model function m10_2(k, t, n)
	alpha ~ filldist(Beta(1, 1), 3)
	beta ~ filldist(Beta(1, 1), 3)
	theta = min.(1, exp.(-alpha * t') .+ beta)
	k .~ Binomial.(n, theta')
end;

# ╔═╡ bb239afc-28a2-4171-99f0-40daef8a56d9
chns_m10_2 = sample(m10_2(k, t, n), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 1a865efa-c04f-4d1a-b821-9e2c395764a0
describe(chns_m10_2)

# ╔═╡ 3852b215-7c51-49bd-99cc-de6b1b5bea15
plot(chns_m10_2)

# ╔═╡ 558219cc-b16f-4196-9bf3-86ea91eb0974
begin
	post_alpha1 = chns_m10_2["alpha[1]"][:];
	post_alpha2 = chns_m10_2["alpha[2]"][:];
	post_alpha3 = chns_m10_2["alpha[3]"][:];
	post_beta1 = chns_m10_2["beta[1]"][:];
	post_beta2 = chns_m10_2["beta[2]"][:];
	post_beta3 = chns_m10_2["beta[3]"][:];
end;

# ╔═╡ 11052638-14ce-4af3-8e44-7a4ef839ecdb
begin
	scatter(post_alpha1, post_beta1, xlabel = "Decay Rate", ylabel = "Baseline",
		xlims = [-0.1, 1.1], ylims = [-0.1, 1.1], label = "Subject 1", 
		alpha = 0.5, title = "Posterior Plot")
	scatter!(post_alpha2, post_beta2, xlabel = "Decay Rate", ylabel = "Baseline",
		label = "Subject 2", alpha = 0.5)
	scatter!(post_alpha3, post_beta3, xlabel = "Decay Rate", ylabel = "Baseline",
		label = "Subject 3", alpha = 0.5)
end

# ╔═╡ 517db81c-da3b-4659-a092-5d7215c7c3be
begin
	post_k1 = Matrix{Int64}(undef, 4000, 10);
	post_k2 = Matrix{Int64}(undef, 4000, 10);
	post_k3 = Matrix{Int64}(undef, 4000, 10);
	for i in 1:4000
		for j in 1:10
			post_k1[i, j] = rand(Binomial(18, exp.(-post_alpha1[i] * t2[j]) + post_beta1[i]));
			post_k2[i, j] = rand(Binomial(18, exp.(-post_alpha2[i] * t2[j]) + post_beta2[i]));
			post_k3[i, j] = rand(Binomial(18, exp.(-post_alpha3[i] * t2[j]) + post_beta3[i]));
		end;
	end;
end;

# ╔═╡ 7769c8a3-f1d1-46b8-bffd-4e68c458b1cf
begin
	p1 = errorline(1:10, post_k1, linewidth = 3, ylims = [0, 20], label = "Fitted",
		title = "Subject 1", xlabel = "Time Lags", ylabel = "Retention Count",
		xticks = (1:10, [i for i in t2]))
	plot!(1:9, k[:,1],  linewidth = 3, label = "Raw")
	p2 = errorline(1:10, post_k2, linewidth = 3, ylims = [0, 20], label = "Fitted",
		title = "Subject 2", xlabel = "Time Lags", ylabel = "Retention Count",
		xticks = (1:10, [i for i in t2]))
	plot!(1:9, k[:,2],  linewidth = 3, label = "Raw")
	p3 = errorline(1:10, post_k3, linewidth = 3, ylims = [0, 20], label = "Fitted",
		title = "Subject 3", xlabel = "Time Lags", ylabel = "Retention Count",
		xticks = (1:10, [i for i in t2]))
	plot!(1:9, k[:,3],  linewidth = 3, label = "Raw")
	plot(p1, p2, p3, layout = (1, 3))
end

# ╔═╡ 6ee72fc8-2bda-4325-8353-c5ab69036744
md"## 10.3. Structured individual differences"

# ╔═╡ 5dd46e92-4a31-4633-99dd-8e133dadced6
@model function m10_3(k, t, n)
	alphamu ~ Beta(1, 1)
	alphalambda ~ Gamma(0.001, 0.001)
	betamu ~ Beta(1, 1)
	betalambda ~ Gamma(0.001, 0.001)
	alpha ~ filldist(truncated(Normal(alphamu, alphalambda), 0, 1), 3)
	beta ~ filldist(truncated(Normal(betamu, betalambda), 0, 1), 3)
	theta = min.(1, exp.(-alpha * t') .+ beta)
	k .~ Binomial.(n, theta')
end;

# ╔═╡ 6c099f6b-b76f-4465-b138-44b19cd8c0b8
chns_m10_3 = sample(m10_3(k, t, n), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ f4e2884b-405c-477e-adcd-d596e791590b
describe(chns_m10_3)

# ╔═╡ 1a8f9908-d77c-408b-b884-e70ec7032e19
begin
	post_alphamu = chns_m10_3["alphamu"][:];
	post_alphalambda = chns_m10_3["alphalambda"][:];
	post_betamu = chns_m10_3["betamu"][:];
	post_betalambda = chns_m10_3["betalambda"][:];
end;

# ╔═╡ 4623d1fc-db56-49b9-85d3-65250205051c
begin
	post_ks = Matrix{Int64}(undef, 4000, 10);
...
# ╔═╡ e993ef92-3ea3-4b06-b971-e762d8fbc92c
begin
	errorline(1:10, post_ks, linewidth = 3, ylims = [0, 20], label = "Fitted k",
		title = "Posterior Plot", xlabel = "Time Lags", ylabel = "Retention Count",
		xticks = (1:10, [i for i in t2]))
	plot!(1:9, k, linewidth = 3,
		label = ["Subject 1" "Subject 2" "Subject 3"])
	scatter!(1:9, k, label = "")
end
