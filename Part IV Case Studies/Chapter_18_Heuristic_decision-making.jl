...
# ╔═╡ 7b7a7713-6685-4ae0-a8f1-c6a90bf4379c
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 75fa1dfa-34f8-11ee-0c1e-d9a05b845506
md"# 18. Heuristic decision-making"

# ╔═╡ e7db425c-8a1c-4698-a91e-e4c691fffe40
md"## 18.1. Take-the-best"

# ╔═╡ 46a01db3-c99c-4c38-9477-e6485ce334af
@model function m18_1(y, t)
	gamma ~ Uniform(0.5, 1)
	ttb = [1 - gamma 0.5 gamma]
	y .~ Bernoulli.(ttb[t])
end;

# ╔═╡ e59a23e2-f223-4076-8482-87647d1bdfb6
# Data
begin
	data = matopen("/Users/semih/Desktop/StopSearchData.mat");
	y = read(data, "y")';
	y = convert(Matrix{Int64}, y);
	m = read(data, "m");
	p = read(data, "p");
	p = convert(Matrix{Int64}, p);
	v = read(data, "v");
	x = read(data, "x");
	s = sortperm(read(data, "v"), dims = 1);
	n, nc = size(m);
	nq = size(p)[1];
	ns = size(y)[2];
end;

# ╔═╡ 4568ef30-4875-4dc3-a486-7241cab5abd6
# Data preparation
begin
	tmp1 = Matrix{Int64}(undef, nq, nc);
	tmp2 = Vector{Int64}(undef, nq);
	tmp3 = Vector{Int64}(undef, nq);
	t = Vector{Int64}(undef, nq);
	for q in 1:nq
		for j in 1:nc
			tmp1[q, j] = (m[p[q, 1], j] - m[p[q, 2], j]) * 2^(s[j] - 1)
		end;
		tmp2[q] = sum(tmp1[q, 1:nc])
		tmp3[q] = -1 * Float64(-tmp2[q] > 0) + Float64(tmp2[q] > 0)
		t[q] = tmp3[q] + 2
	end;
	t = reshape(t, 30, 1);
end;

# ╔═╡ e6ef860e-7cb6-41fd-95af-4bce82d6de44
chns_m18_1 = sample(m18_1(y, t), NUTS(), MCMCThreads(), 1000 , 4);

# ╔═╡ f12e0a85-4c5a-4284-83ee-9648c70204b4
describe(chns_m18_1)

# ╔═╡ f20904ab-d176-4f2f-b2ac-8128e7352c24
plot(chns_m18_1)

# ╔═╡ 049718c2-1001-468a-9735-45e7295cefa0
md"## 18.2. Stopping"

# ╔═╡ d3d770f4-c4f9-4f06-83d3-5bd417e38b10
@model function m18_2(y, t1, t2)
	phi ~ Uniform(0, 1)
	z ~ filldist(Bernoulli(phi), 20)
	z1 = reshape(repeat(z, 30), 20, 30)
	gamma ~ Uniform(0.5, 1)
	dec = [1 - gamma 0.5 gamma]
	t = t1 .* (z1 .== 1) + t2 .* (z1 .== 0)
	y .~ Bernoulli.(dec[t])
end;

# ╔═╡ 2dbfba87-4176-4100-b2fc-42cd035055fc
# Data preparation (TTB)
begin
	tmp1_2 = Array{Int64}(undef, ns, nq, nc);
	tmp2_2 = Matrix{Int64}(undef, ns, nq);
	tmp3_2 = Matrix{Int64}(undef, ns, nq);
	t_2 = Array{Int64}(undef, ns, nq, nc);
	for i in 1:ns
		for q in 1:nq
			for j in 1:nc
				tmp1_2[i, q, j] = (m[p[q, 1], j] - m[p[q, 2], j]) * 2^(s[j] - 1)
			end;
			tmp2_2[i, q] = sum(tmp1_2[i, q, 1:nc])
			tmp3_2[i, q] = -1 * Float64(-tmp2_2[i, q] > 0) + Float64(tmp2_2[i, q] > 0)
			t_2[i, q, 1] = tmp3_2[i, q] + 2
		end;
	end;
end;

# ╔═╡ 62ec39b4-cbb9-4e0c-b9ca-bf0451b0b9fc
# Data preparation (WADD)
begin
	tmp4 = Array{Float64}(undef, ns, nq, nc);
	tmp5 = Matrix{Float64}(undef, ns, nq);
	tmp6 = Matrix{Float64}(undef, ns, nq);
	for i in 1:ns
		for q in 1:nq
			for j in 1:nc
			...
			tmp6[i, q] = -1 * Float64(-tmp5[i, q] > 0) + Float64(tmp5[i, q] > 0)
			t_2[i, q, 2] = tmp6[i, q] + 2
		end;
	end;
	t_ttb = t_2[:, :, 1];
	t_wadd = t_2[:, :, 2];
...
# ╔═╡ e488e119-670b-4511-bf52-b2ce70db1fe6
chns_m18_2 = sample(m18_2(y1, t_ttb, t_wadd), MH(), MCMCThreads(), 1000, 4);

# ╔═╡ 4b617a89-d604-4102-b96c-9c20673ebe00
summarystats(chns_m18_2[["gamma", "phi"]])

# ╔═╡ f3a7a522-9874-4bcd-af0d-9e8278d36e50
md"## 18.3. Searching"

# ╔═╡ 0ce32e28-a7a4-403a-a788-468e8fa044fa
@model function m18_3(y)
	gamma ~ Uniform(0.5, 1)
	tmp1 = Array{Int64}(undef, 20, 30, 9)
	tmp2 = Matrix{Int64}(undef, 20, 30)
	tmp3 = Matrix{Int64}(undef, 20, 30)
	t = Matrix{Int64}(undef, 20, 30)
	stmp ~ filldist(truncated(Normal(0, 1), 0, Inf), 20, 9)
	for i in 1:20
    	for q in 1:30
			s = Matrix{Int64}(undef, 20, 9)
    		for j in 1:9
				s[i, :] = sortperm(stmp[i, 1:9]);
			end;
      		for j in 1:9
        		tmp1[i, q, j] = (m[p[q, 1], j] - m[p[q, 2], j]) * 2^(s[i, j] - 1)
			end;
      		tmp2[i, q] = sum(tmp1[i, q, 1:9])
      		tmp3[i, q] = -1 * Float64(-tmp2[i, q] > 0) + Float64(tmp2[i, q] > 0)
      		t[i, q] = tmp3[i, q] + 2
		end;
	end;
	ttb = [1 - gamma 0.5 gamma]
...
# ╔═╡ 0198d60e-311b-467d-afa4-bdee60742118
chns_m18_3 = sample(m18_3(y1), MH(), MCMCThreads(), 1000, 4);

# ╔═╡ 4f269056-fe5e-4eb6-af4d-40452c182616
summarystats(chns_m18_3[["gamma"]])

# ╔═╡ 5d72918e-6970-4838-be74-a009e5f256d0
md"## 18.4. Searching and stopping"

# ╔═╡ ef022bd0-0888-4ea0-955e-16647704a6a0
@model function m18_4(y, x, m, p)
	phi ~ filldist(Uniform(0, 1), 20)
	z = Matrix{Int64}(undef, 20, 30)
	for i in 1:20
		z[i, :] ~ filldist(Bernoulli(phi[i]), 30)
	end;
	z1 = z .+ 1
	gamma ~ Uniform(0.5, 1)
	tmp1 = Array{Int64}(undef, 20, 30, 9)
	...
	tmp4 = Array{Float64}(undef, 20, 30, 9);
	tmp5 = Matrix{Float64}(undef, 20, 30);
	tmp6 = Matrix{Float64}(undef, 20, 30);
	for i in 1:20
		for q in 1:30
			for j in 1:9
				tmp4[i, q, j] = (m[p[q, 1], j] - m[p[q, 2], j]) * x[j]
			end;
			tmp5[i, q] = sum(tmp4[i, q, 1:9])
			tmp6[i, q] = -1 * Float64(-tmp5[i, q] > 0) + Float64(tmp5[i, q] > 0)
			t[i, q, 2] = tmp6[i, q] + 2
		end;
	end;
	...
	for i in 1:20
		for q in 1:30
			y[i, q] ~ Bernoulli(dec[t[i, q, z1[i, q]]])
		end;
	end;
end;

# ╔═╡ 07eed386-0ce8-4a1b-9eed-98102551d0dc
chns_m18_4 = sample(m18_4(y1, x, m, p), MH(), MCMCThreads(), 1000, 4);

# ╔═╡ fc6f2991-2a67-466b-a5a8-6ebac7aebaa6
summarystats(chns_m18_4[["gamma"]])
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "2e1ff89c89acac9b0d2be88edc30f34b2782713a"
