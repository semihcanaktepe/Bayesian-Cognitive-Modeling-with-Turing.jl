...
@model function m17_2(y, t, d1, d2, a)
	c ~ filldist(Uniform(0, 5), 1, 1, 40)
	w ~ filldist(Uniform(0, 1), 1, 1, 40)
	b = 0.5
	s = exp.(-c .* (w .* d1 + (1 .- w) .* d2))
	Sum_as = sum(a .* s, dims = 2)
	Sum_mas = sum((1 .- a) .* s, dims = 2)
...
# ╔═╡ 86d87bb5-3451-4ef7-9fe7-3b8dab1dc8da
# Prepare the data
begin
	y2 = x';
	a2 = repeat(2 .- a, 1, 8, 40);
	d1_t = repeat(d1, 1, 1, nsubj);
	d2_t = repeat(d2, 1, 1, nsubj)
end;

# ╔═╡ 5d6b96ac-fb5e-4253-b6a0-57c4749c89f6
# chns_m17_2 = sample(m17_1(y2, t, d1_t, d2_t, a2), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 3d3682c1-b426-4e90-bbe8-263ad4b5ea2a
# 17.3. Latent groups in the GCM

# ╔═╡ f73efda4-ea46-4fad-b584-919b2ac01817
@model function m17_3(y, t, d1, d2, a)
	muw1 ~ Uniform(0, 1)
	delta ~ Uniform(0, 1)
	muw2 = min(1, delta + muw1)
	muc ~ Uniform(0, 1)
    sigmac ~ Uniform(0.01, 3)
	sigmaw ~ Uniform(0.01, 1)
	phic ~ Uniform(0, 1)
	phig ~ Uniform(0, 1)
    zc ~ filldist(Bernoulli(phic), 40)
    zg ~ filldist(Bernoulli(phig), 40)
	c ~ filldist(truncated(Normal(muc, sigmac), 0, 5), 1, 1, 40)
	muw = @. muw1 * (zg .== 0) .+ muw2 * (zg .== 1)
	w ~ filldist(truncated(Normal(muw, sigmaw), 0, 1), 1, 1, 40)
	b = 0.5
	s = exp.(-c .* (w .* d1 + (1 .- w) .* d2))
	Sum_as = sum(a .* s, dims = 2)
	Sum_mas = sum((1 .- a) .* s, dims = 2)
	r1 = (b * Sum_as) ./ (b * Sum_as + (1 - b) * Sum_mas)
	r2 = ones(8, 40) * 0.5
	r = @. r1 .* (zk .== 0) .+ r2 .* (zk .== 1) 
	y .~ Binomial.(8, r)
end;
...
# chns_m17_3 = sample(m17_3(y2, t, d1_t, d2_t, a2), NUTS(), MCMCThreads(), 1000, 4);
