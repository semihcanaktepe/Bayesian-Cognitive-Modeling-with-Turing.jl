...
# ╔═╡ 9b1d3d14-60a0-484a-87ea-958489b4fea8
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 148adf50-3217-11ee-2b8f-df2fd6422077
md"# 12. Psychophysical functions"

# ╔═╡ 93f11f82-1aeb-4317-9421-2acf1467d2b8
# define logit and inverse logit functions
begin
	# logit
	function logit(p::Real)
		# p is a real between 0 and 1
		return log(p / (1 - p))
	end;
	
	# inverse logit
	function invlogit(x::Real)
		return exp(x) / (1 + exp(x))
	end;
end;

# ╔═╡ 75c9721a-ec5d-4def-ae9a-77161206acc3
md"## 12.1. Psychophysical functions"

# ╔═╡ dd30e262-40cb-4051-bc06-916797bcbdf3
@model function m12_1(r, n, x, xbar, subjs)
	mua ~ Normal(0, 0.001)
	mub ~ Normal(0, 0.001)
	sigmaa ~ Uniform(0, 1000)
	sigmab ~ Uniform(0, 1000)
	alpha ~ filldist(Normal(mua, sigmaa), length(unique(subjs)))
	beta ~ filldist(Normal(mub, sigmab), length(unique(subjs)))
	logit_theta = alpha[subjs] + beta[subjs] .* (x - xbar)
	r .~ BinomialLogit.(n, logit_theta)
end;

# ╔═╡ 8ca2988e-a53a-40da-864f-8f07829d9c4a
# Data
begin
	rdat = CSV.read("/Users/semih/Desktop/data_r.txt", delim = "\t", 
		header = false, missingstring = "NA", DataFrame);
	ndat = CSV.read("/Users/semih/Desktop/data_n.txt", delim = "\t", 
		header = false, missingstring = "NA", DataFrame);
	xdat = CSV.read("/Users/semih/Desktop/data_x.txt", delim = "\t", 
		header = false, missingstring = "NA", DataFrame);
	rprop = CSV.read("/Users/semih/Desktop/data_rprop.txt", delim = "\t", 
		header = false, missingstring = "NA", DataFrame);
end;

# ╔═╡ be4da575-74cf-416c-933b-4885954cd3e1
...
# ╔═╡ 69d3020a-3f1b-4feb-884a-88f0861de87a
# Create a mean array of x named xmeans
begin
	xmeans = Vector{Float64}(undef, size(xd)[1]);
	# there are 8 subjects
	for i in 1:size(xd)[1]
		xmeans[i] = mean(skipmissing(xd[i,:]))
	end;
end;

# ╔═╡ 4fc0568c-e561-4e30-8668-04598429cdb0
# Print subject means
xmeans

# ╔═╡ fd074ae3-d61f-40eb-be0d-bbdf2a4b713a
# Convert from matrix format to long format

# ╔═╡ a7ac1f50-8e62-45fc-8977-b58c34a9f82d
# Keep only the non-missing values
nonmissing2 = .!ismissing.(xd);

# ╔═╡ 326f43ba-8ee6-41e4-80af-3ebdbdbcdc57
nonmissing = nonmissing2[:]

# ╔═╡ 7e33e247-12c7-4ced-8242-fcad2a6d856c
# Convert data matrices into vectors of non-missing values
begin
	r = rd[nonmissing];
	n = nd[nonmissing];
	x = xd[nonmissing];
...
# ╔═╡ 9ede5a9e-dcd9-44c9-8d88-d412d0e15271
# Extend the xmeans vector as x is also converted into a vector
begin
	xbar2 = repeat(collect(xmeans),  size(rd)[2]);
	xbar = xbar2[nonmissing];
end;

# ╔═╡ 96fab3be-c236-44de-9eea-adebbb961b1d
# Keep the subject ids since we lost the row and column orders in vectorizing
begin
	subjs2 = repeat(collect(1:size(rd)[1]), size(rd)[2]);
	subjs = subjs2[nonmissing];
end;

# ╔═╡ a8a4c1eb-9d9e-4798-87d8-2f5f69268af5
#chns_m12_1 = sample(m12_1(r, n, x, xbar, subjs), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ aa565c0c-fc4e-4e6f-95a6-f0a81debf8c8
describe(chns_m12_1)

# ╔═╡ c95037c6-6bac-44aa-aa23-c5a0cb7b0cca
plot(1:28, [rpropd[i, :] for i in 1:8], layout = (2, 4), 
	title = "Subject ", label = false, xtick = false,
	ytick = [0, 0.5, 0.84, 1.0], linewidth = 2)

# ╔═╡ f03bbf3b-3889-4c9e-8d6b-d0b540d68546
md"## 12.2. Psychophysical functions under contamination"

# ╔═╡ e52ee77c-fa16-4379-a111-d1e739a62525
# Define probit function
function Phi(x::Real)
    return 0.5 + 0.5 * erf(x / sqrt(2))
end;

# ╔═╡ 54c3b82a-77c8-4839-8b15-2f0dd06ccb7f
@model function m12_2(r, n, x, xbar, subjs)
	mua ~ Normal(0, 0.001)
	mub ~ Normal(0, 0.001)
	muphi ~ Normal(0, 0.001)
	sigmaa ~ Uniform(0, 1000)
	sigmab ~ Uniform(0, 1000)
	sigmaphi ~ Uniform(0, 3)
	# As pi will pass to BinomialLogit, Normal distribution is used.
	# pi ~ filldist(Uniform(0, 1), length(r))
	logit_pi ~ filldist(Normal(0, 1), length(r))
	probit_phi ~ filldist(Normal(muphi, sigmaphi), length(unique(subjs)))
	phi = Phi.(probit_phi)
	# z .~ Bernoulli.(phi[subjs])
	z_ = rand(length(r))
	z = @. (z_ < phi[subjs])
	alpha ~ filldist(Normal(mua, sigmaa), length(unique(subjs)))
	beta ~ filldist(Normal(mub, sigmab), length(unique(subjs)))
	logit_theta = @. (alpha[subjs] + beta[subjs] .* (x - xbar)) * (z .== 0) +
		logit_pi * (z .== 1)
	r .~ BinomialLogit.(n, logit_theta)
end;

# ╔═╡ edb732bd-db61-42d4-b982-f4b6d651c991
chns_m12_2 = sample(m12_2(r, n, x, xbar, subjs), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 1cf3ce86-3bf6-48a0-954d-48c7595e34be
describe(chns_m12_2)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "b4f379e3cbc82bce42d8741d074cfca593d97430"
