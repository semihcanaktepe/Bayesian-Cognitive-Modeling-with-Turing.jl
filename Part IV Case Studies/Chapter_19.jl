...
# ╔═╡ 6382c8b2-2d6d-465e-8b72-3257b2f266d3
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ e51e742a-353d-11ee-062e-37473a8c5440
# 19. Number concept development

# ╔═╡ c15172e8-95ed-4727-84e7-4ff4f698b76c
# 19.1. Knower-level model for Give-N

# ╔═╡ 935d64ad-656a-4dcc-9177-d7db4f306808
@model function m19_1(ga, gq, ind, validind)
	pi ~ filldist(Dirichlet(ones(15)), 15)
	v ~ Uniform(1, 1000)
	vvec = [1.0 1.0 / v v]
	piprime = vvec[ind] .* pi
	npiprime = piprime ./ sum(piprime, dims = 3)
	z ~ filldist(Categorical(ones(6)/6), 20)
	z_vec = repeat(z, 21)
	pi_ = npiprime[z_vec[validind], gq[validind], :]
	a = ga[validind]
	a .~ Categorical.(pi_)
end;

# ╔═╡ d32bd7db-9551-48ca-bc6b-70d4c5e44c73
# Data
begin
	data = matopen("/Users/semih/Desktop/fc_given.mat");
	ns = convert(Int64, read(data, "ns"));
	nz = convert(Int64, read(data, "nz"));
	gnq = convert(Matrix{Int64}, read(data, "gnq"));
	gn = convert(Int64, read(data, "gn"));
	ga = convert(Matrix{Int64}, read(data, "ga"));
	gq = convert(Matrix{Int64}, read(data, "gq"));
end;

# ╔═╡ 5aed73a7-568a-428d-a354-a85ec6f0aca3
# Data preparation
begin
	ind5 = convert(Array{Int64}, zeros(nz, gn, gn));
	for i in 1:nz
    	i1 = i
    	for j in 1:gn
        	j1 = j
	...
	ga_obs = ga[:] .- 1;
	gq_obs = gq[:] .- 1;
	valid_ind = findall(x -> x != -1, gq_obs);
end;

# ╔═╡ 2d29d28e-6555-4841-8e0b-adfe303f04f1
# chns_m19_1 = sample(m19_1(ga_obs, gq_obs, ind5r, valid_ind), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ f05329ea-34d1-4f42-aa01-e4e4fc2d1133
# 19.2. Knower-level model for Fast-Cards

# ╔═╡ ac4491d1-cf9d-4f5b-8d6c-21c6ab63f2fa
@model function m19_2(fa, fq, ind, validind)
	pi ~ filldist(Dirichlet(ones(50)), 50)
	v ~ Uniform(1, 1000)
	vvec = [1.0 1.0 / v v]
	piprime = vvec[ind] .* pi
	npiprime = piprime ./ sum(piprime, dims = 3)
	z ~ filldist(Categorical(ones(6)/6), 20)
	z_vec = repeat(z, 21)
	pi_ = npiprime[z_vec[validind], fq[validind], :]
	a = fa[validind]
	a .~ Categorical.(pi_)
end;

# ╔═╡ dde46654-2f4f-4b3e-8afe-5f067bcf93ab
# Data
begin
	fnq = convert(Matrix{Int64}, read(data, "fnq"));
	fa = convert(Matrix{Int64}, read(data, "fa"));
	fq = convert(Matrix{Int64}, read(data, "fq"));
	fn = 50;
end;

# ╔═╡ 7d100370-2ccb-4f1e-b425-29cacfb2556f
# Data preparation
begin
	find5 = convert(Array{Int64}, zeros(nz, gn, fn));
	for i in 1:nz
    	for j in 1:gn
        	for k in 1:gn
            	# Will be 1 if Knower-Level is Same or Greater than Answer
            	...
            	find4 = Int64(i == nz)
            	find5[i, j, k] =
					find3 + find4 * (2 + find2) + (1 - find4) * (1 - find3) * (find1 * find2 + find1 + 1)
			end;
		end;
	end;
	find5r = ind5 .- 1;
	fa_obs = fa[:] .- 1;
	fq_obs = fq[:] .- 1;
	valid_ind2 = findall(x -> x != -1, fq_obs);
end;

# ╔═╡ 9ace8bc4-b162-432d-ae2d-4a03c9003b61
# chns_m19_2 = sample(m19_2(fa_obs, fq_obs, find5r, valid_ind2), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 52c5ae63-1aca-47c2-9d72-61f1e2ebd15f
# 19.3. Knower-level model for Give-N and Fast-Cards

# ╔═╡ 09c19993-2edf-4be8-9d8b-119e8bd7883e
@model function m19_3(ga, fa, gq, fq, ind1, ind2, validind1, validind2)
	# Knower level
	z ~ filldist(Categorical(ones(6)/6), 20)
	# Give-N
	pig ~ filldist(Dirichlet(ones(15)), 15)
	vg ~ Uniform(1, 1000)
	vgvec = [1.0 1.0 / vg vg]
	piprimeg = vgvec[ind1] .* pig
	npiprimeg = piprimeg ./ sum(piprimeg, dims = 3)
	zg_vec = repeat(z, 21)
	pig_ = npiprimeg[zg_vec[validind1], gq[validind1], :]
	ag = ga[validind1]
	ag .~ Categorical.(pig_)
	# Fast-Cards
	pif ~ filldist(Dirichlet(ones(50)), 50)
	vf ~ Uniform(1, 1000)
	vfvec = [1.0 1.0 / vf vf]
	piprimef = vfvec[ind2] .* pif
	npiprimef = piprimef ./ sum(piprimef, dims = 3)
	zf_vec = repeat(z, 21)
	pif_ = npiprimef[zf_vec[validind2], fq[validind2], :]
	af = fa[validind2]
	af .~ Categorical.(pif_)
end;

# ╔═╡ 08f45247-16af-4bbd-840a-91816a781931
# chns_m19_3 = sample(m19_3(ga_obs, fa_obs, gq_obs, fq_obs, ind5r, find5r, valid_ind, valid_ind2), NUTS(), MCMCThreads(), 1000, 4);
