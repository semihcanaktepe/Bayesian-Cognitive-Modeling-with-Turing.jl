...
# ╔═╡ d70687a4-7f8d-427e-bf35-3b6572558b50
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ e40307e8-31fc-11ee-0939-f99e4c5167a5
md"# 11. Signal detection theory"

# ╔═╡ a3f3b5a8-98ea-4fc2-bcdb-a46510f5770e
# define phi
function phi(x::Real)
	return 0.5 + 0.5 * erf(x / sqrt(2))
end;

# ╔═╡ 5aa5af6b-9fde-45db-8e9c-7adb78a1954a
md"## 11.1. Signal detection theory"

# ╔═╡ d3756068-7452-4f45-977f-6fed42b1044c
@model function m11_1(h, f, s, n)
	d ~ filldist(Normal(0, 0.5), length(h))
	c ~ filldist(Normal(0, 2), length(f))
	thetah = phi.(d / 2 - c)
	thetaf = phi.(-d / 2 - c)
	h .~ Binomial.(s, thetah)
	f .~ Binomial.(n, thetaf)
...
# ╔═╡ 7c199202-8584-4e40-ac30-89a6e7a7220b
# Data
begin
	data = reshape([70, 50, 30, 50, 7, 5, 3, 5, 10, 0, 0, 10], 3, 4);
	h = data[:, 1];
	f = data[:, 2];
	mi = data[:, 3];
	cr = data[:, 4];
...
# ╔═╡ 550b15b6-5994-40a9-a36c-573315531d8d
chns_m11_1 = sample(m11_1(h, f, s, n), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 28e126cc-d1bc-4b4b-9a97-3a0764beb5b6
describe(chns_m11_1)

# ╔═╡ 1290da55-cbe8-4a47-a93d-6e62c7575baf
plot(chns_m11_1)

# ╔═╡ aaf8c0d1-0ac4-4285-8f97-1c97b06ed3f4
begin
	d1 = chns_m11_1["d[1]"][:];
	d2 = chns_m11_1["d[2]"][:];
	d3 = chns_m11_1["d[3]"][:];
...
# ╔═╡ 9904127a-d63f-4b39-af84-c776acccaf34
begin
	p1 = density(d1, linewidth = 3, label = "Case 1",
		xlabel = "Discriminability", ylabel = "Probability Density")
		density!(d2, linewidth = 3, label = "Case 2")
		density!(d3, linewidth = 3, label = "Case 3")
	p2 = density(c1, linewidth = 3, label = "Case 1",
		xlabel = "Bias", ylabel = "Probability Density")
		density!(c2, linewidth = 3, label = "Case 2")
		density!(c3, linewidth = 3, label = "Case 3")
	p3 = density(h1, linewidth = 3, label = "Case 1",
		xlabel = "Hit Rate", ylabel = "Probability Density")
		density!(h2, linewidth = 3, label = "Case 2")
		density!(h3, linewidth = 3, label = "Case 3")
	p4 = density(f1, linewidth = 3, label = "Case 1",
		xlabel = "False Alarm", ylabel = "Probability Density")
		density!(f2, linewidth = 3, label = "Case 2")
		density!(f3, linewidth = 3, label = "Case 3")
	plot(p1, p2, p3, p4, layout = (2, 2))
end

# ╔═╡ 5c0a0614-3073-41d0-a641-23b27c83f1fd
md"## 11.2. Hierarchical signal detection theory"

# ╔═╡ d375672b-4d62-455f-8c48-e68689951ee8
@model function m11_2(h, f, s, n)
	mud ~ Normal(0, 0.001)
	muc ~ Normal(0, 0.001)
	lambdad ~ Gamma(0.001, 0.001)
  	lambdac ~ Gamma(0.001, 0.001)
	d ~ filldist(Normal(mud, lambdad), length(h))
	c ~ filldist(Normal(muc, lambdac), length(f))
	thetah = phi.(0.5 * d - c)
	thetaf = phi.(-0.5 * d - c)
	h .~ Binomial.(s, thetah)
	f .~ Binomial.(n, thetaf)
...
# ╔═╡ 2d8eff09-0c73-44e5-85a3-814e3572c7e4
# Data
begin
	# induction data
	induc = CSV.read("/Users/semih/Desktop/induction.csv", DataFrame);
	# deduction data
	deduc = CSV.read("/Users/semih/Desktop/deduction.csv", DataFrame);
end;

# ╔═╡ af7603f0-2219-4fe4-a900-e6143cfac794
# Adjust the data
begin
	# induction data
	hi = induc[!, "V1"];
...
# ╔═╡ cec90f97-4aea-4392-826c-f209e8e4bc89
# sample using induction data
chns_m11_2_i = sample(m11_2(hi, fi, si, ni), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ aacfd2ee-a627-4f4e-b6c7-fc4df98c4682
describe(chns_m11_2_i)

# ╔═╡ e17ecea3-1f7e-4753-8547-cdecda12c229
# sample using deduction data
chns_m11_2_d = sample(m11_2(hd, fd, sd, nd), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 7fb5ed55-5e5e-41c4-a47f-3aa90c69c28e
describe(chns_m11_2_d)

# ╔═╡ 121108e9-550b-4c58-8ccb-31544abd0e77
begin
	mudi = chns_m11_2_i["mud"][:];
	muci = chns_m11_2_i["muc"][:];
	mudd = chns_m11_2_d["mud"][:];
	mucd = chns_m11_2_d["muc"][:];
end;

# ╔═╡ 1c9c2147-0514-4a08-8c2d-2f98c99b9165
begin
	p11 = scatter(mudi, muci, label = "Induction", alpha = 0.5)
	scatter!(mudd, mucd, label = "Deduction", alpha = 0.5)
	p12 = density(muci, linewidth = 3, xlabel = "muc", label = "Induction")
	density!(mucd, linewidth = 3, label = "Deduction")
	p13 = density(mudi, linewidth = 3, xlabel = "mud", label = "Induction")
	density!(mudd, linewidth = 3, label = "Deduction")
	plot(p11, p12, p13, layout = (2,2))
end

# ╔═╡ 6b4348fb-dab5-4708-8514-5a186c5b9449
begin
	lambda_i = chns_m11_2_i["lambdac"][:];
	lambda_d = chns_m11_2_d["lambdac"][:];
	sigma_i = 1 ./ lambda_i.^2;
	sigma_d = 1 ./ lambda_d.^2;
end;

# ╔═╡ 2c400dd4-8eee-43bf-9b90-c0aa7fad9b28
begin
	p14 = plot(sigma_i, title = "Induction", xlabel = "MCMC Sample",
		ylabel = "sigma_c", label = "")
	p15 = plot(sigma_d,  title = "Deduction", xlabel = "MCMC Sample",
		ylabel = "sigma_c", label = "")
	plot(p14, p15, layout = (2, 1))
end

# ╔═╡ ea1131db-0006-4994-ba36-b1aa25948f47
md"## 11.3. Parameter expansion"

# ╔═╡ 7cc4bdf9-b2ad-4571-9753-78ce9a157ba5
@model function m11_3(h, f, s, n)
	mud ~ Normal(0, 0.001)
	muc ~ Normal(0, 0.001)
	lambdad ~ Gamma(0.1, 0.1)
  	lambdac ~ Gamma(0.1, 0.1)
	xid ~ Beta(1, 1)
	xic ~ Beta(1, 1)
	deltad ~ filldist(Normal(0, lambdad), length(h))
	deltac ~ filldist(Normal(0, lambdac), length(f))
	sigmad = abs(xid) / sqrt(lambdad)
	sigmac = abs(xic) / sqrt(lambdac)
	d = mud .+ xid * deltad
	c = muc .+ xic * deltac
	thetah = phi.(0.5 * d - c)
	thetaf = phi.(-0.5 * d - c)
...
# ╔═╡ 552e50e4-1aeb-4c33-91d4-e1f5b50e453d
# sample using induction data
chns_m11_3_i = sample(m11_3(hi, fi, si, ni), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 17194846-7c20-4bcd-bfea-b5f76ecde0d2
describe(chns_m11_3_i)

# ╔═╡ d719d3bd-4404-447b-af9c-2419a9f5d35c
# sample using deduction data
chns_m11_3_d = sample(m11_3(hd, fd, sd, nd), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ f39c7fc5-1409-492e-a46d-6e9df2518001
describe(chns_m11_3_d)

# ╔═╡ 3d05f9ac-13b8-4465-9016-34037b65f0c3
begin
	lambda_i3 = chns_m11_3_i["lambdac"][:];
	lambda_d3 = chns_m11_3_d["lambdac"][:];
	sigma_i3 = 1 ./ lambda_i3.^2;
	sigma_d3 = 1 ./ lambda_d3.^2;
end;

# ╔═╡ 6ffeeca8-259a-4fbd-8b80-3753c19d13d4
begin
	p31 = plot(sigma_i3, title = "Induction", xlabel = "MCMC Sample",
		ylabel = "sigma_c", label = "")
	p32 = plot(sigma_d3,  title = "Deduction", xlabel = "MCMC Sample",
		ylabel = "sigma_c", label = "")
	plot(p31, p31, layout = (2, 1))
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "b029b0a3aac257c4f80fc349c6d4fa3df477d6a1"
