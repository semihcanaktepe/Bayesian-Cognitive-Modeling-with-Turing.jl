...
begin
	...
	k = reshape([18, 18, 16, 13, 9, 6, 4, 4, 4, 
		17, 13,  9,  6, 4, 4, 4, 4, 4, 
		14, 10,  6,  4, 4, 4, 4, 4, 4], 3, 9);
	# number of items
	n = 18;
end;

# ╔═╡ 3952a51c-7acb-4d39-b12d-bdc9c9a26a5e
chns_m10_1 = sample(m10_1(k, t, n), NUTS(), MCMCThreads(), 1000, 4);
...
@model function m10_2(k, t, n)
	alpha ~ filldist(Beta(1, 1), size(k)[1])
	beta ~ filldist(Beta(1, 1), size(k)[1])
	theta = min.(1, exp.(-alpha .* t') .+ beta)
	k .~ Binomial.(n, theta)
end;
...
@model function m10_3(k, t, n)
	...
	alpha ~ filldist(truncated(Normal(alphamu, alphalambda), 0, 1), size(k)[1])
	beta ~ filldist(truncated(Normal(betamu, betalambda), 0, 1), size(k)[1])
	theta = min.(1, exp.(-alpha .* t') .+ beta)
	k .~ Binomial.(n, theta)
end;
...
# ╔═╡ 67a31956-dacb-4d6d-bd46-a99b20aa35c9
plot(chns_m10_3)
