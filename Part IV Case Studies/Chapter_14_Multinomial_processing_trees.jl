...
# ╔═╡ b3c2ec89-9c68-45a6-86ab-1c0953d5f1ff
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 48b1b248-329d-11ee-2523-d99cf3d61ba5
md"# 14. Multinomial processing trees"

# ╔═╡ d7d54f84-2448-4315-aca8-f4167be2ab19
md"## 14.1. Multinomial processing model of pair-clustering"

# ╔═╡ 729ed52c-4d68-4e89-b8d2-ae097364dc6a
@model function m14_1(k, n)
	c ~ Beta(1, 1)
	r ~ Beta(1, 1)
	u ~ Beta(1, 1)
	theta1 = c * r
	theta2 = (1 - c) * u^2
	theta3 = 2 * u * (1 − c) * (1 − u)
	theta4 = c * (1 − r) + (1 − c) * (1 − u)^2
	k ~ Multinomial(n, [theta1, theta2, theta3, theta4])
end;

# ╔═╡ 20cfe75b-d6a2-43b0-8991-2cae893a35ac
 # Data
begin
	k = [45 24 97 254; 
		106 41 107 166; 
		243 64 65 48];
	n = Vector{Int64}(undef, size(k)[1])
	for i in 1:size(k)[1]
		n[i] = sum(k[i, :])
	end;
end;

# ╔═╡ e13b7898-0595-4e64-99e3-6262a203aca8
# Run the model for each trial
begin
	chns_m14_1 = Vector{Chains}();
	for i in 1:size(k)[1]
		chns_i = sample(m14_1(k[i, :], n[i]), NUTS(), MCMCThreads(), 1000, 4);
		push!(chns_m14_1, chns_i);
	end;
end;

# ╔═╡ 01a990b7-0c7e-46c3-ba0f-8f91f02e11c5
begin
	post_c = Vector();
	post_r = Vector();
	post_u = Vector();
	for i in 1:3
		post_ci = chns_m14_1[i]["c"][:];
		post_ri = chns_m14_1[i]["r"][:];
		post_ui = chns_m14_1[i]["u"][:];
		push!(post_c, post_ci);
		push!(post_r, post_ri);
		push!(post_u, post_ui);
	end;
	...
	u_pdf = Vector();
	for i in 1:3
		c_pdf_i = kde(post_c[i])
		r_pdf_i = kde(post_r[i])
		u_pdf_i = kde(post_u[i])
		push!(c_pdf, c_pdf_i);
		push!(r_pdf, r_pdf_i);
		push!(u_pdf, u_pdf_i);
	end;
...
# ╔═╡ b21e552f-a202-4cc8-83cb-bf8946e4a79b
begin
	p1 = plot(x1, [pdf(c_pdf[i], x1) for i in 1:3], 
		label = ["Trial 1" "Trial 2" "Trial 6"], 
		xlabel = "c", ylabel = "Probability Density", 
		linewidth = 3, xlims = [0, 1])
	p2 = plot(x1, [pdf(r_pdf[i], x1) for i in 1:3], 
		label = ["Trial 1" "Trial 2" "Trial 6"], 
		xlabel = "r", ylabel = "Probability Density", 
		linewidth = 3, xlims = [0, 1])
	p3 = plot(x1, [pdf(u_pdf[i], x1) for i in 1:3], 
		label = ["Trial 1" "Trial 2" "Trial 6"], 
...
# ╔═╡ 527e6437-419a-4fad-91c9-60f1b27ddbac
md"## 14.2. Latent-trait MPT model"

# ╔═╡ 9385397e-17c5-4ea8-8289-470229fcc425
# Define probit function
function Phi(x::Real)
	0.5 + 0.5 * erf(x / sqrt(2))
end;

# ╔═╡ 38946089-4709-4022-b80e-5e910ea20099
@model function m14_2(k, n)
	muchat ~ Normal(0, 1)
	murhat ~ Normal(0, 1)
	muuhat ~ Normal(0, 1)
	xichat ~ Uniform(0, 100)
	xirhat ~ Uniform(0, 100)
	xiuhat ~ Uniform(0, 100) 
	# Wishart covariance matrix
	#=
	SigmaInv ~ InverseWishart(4, Matrix(I, 3, 3))
	cov = Hermitian(inv(SigmaInv))
	=#
	# LKJ correlation matrix
	Rho ~ LKJ(3, 2.0)
	Sigma ~ filldist(Exponential(1), 3)
	K = Sigma .* Rho .* Sigma'
	cov = (K' .+ K) / 2
	deltahat ~ filldist(MvNormal(zeros(3), cov), 21)
	deltachat = deltahat[1, :]
	deltarhat = deltahat[2, :]
	deltauhat = deltahat[3, :]
	c = Phi.(muchat .+ xichat * deltachat)
	r = Phi.(murhat .+ xirhat * deltarhat)
	u = Phi.(muuhat .+ xiuhat * deltauhat)
	theta1 = c .* r
    theta2 = (1 .- c) .* u.^2
    theta3 = 2 .* u .* (1 .- c) .* (1 .- u)
    theta4 = c .* (1 .- r) .+ (1 .- c) .* (1 .- u).^2
	for i in 1:21
		k[i, :] ~ Multinomial(n[i], [theta1[i], theta2[i], theta3[i], theta4[i]])
	end;
end;

# ╔═╡ 521101a1-b7f9-40d7-86c9-84e92e00f8bc
# Data
begin
	response1 = reshape([2,4,4,10,2,1,3,14,2,2,5,11,6,0,4,
		10,1,0,4,15,1,0,2,17,1,2,4,13,4,1,6,9,5,
		1,4,10,1,0,9,10,5,0,3,12,0,1,6,13,1,5,7,
		7,1,1,4,14,2,2,3,13,2,1,5,12,2,0,6,12,1,
		0,5,14,2,1,8,9,3,0,2,15,1,2,3,14], 21, 4);
	response2 = reshape([7,5,3,5,5,2,3,10,6,2,7,5,9,4,2,5,2,2,7,
		9,1,3,3,13,5,0,5,10,7,3,4,6,7,3,6,4,4,1,
        10,5,9,1,2,8,3,1,6,10,3,5,9,3,2,0,6,12,
        8,0,3,9,3,2,7,8,7,1,5,7,2,1,6,11,5,3,5,
        7,5,0,6,9,6,2,2,10], 21, 4);
	response6 = reshape([14,3,1,2,12,3,1,4,18,0,1,1,15,3,0,2,7,
		1,10,2,3,6,11,0,8,4,3,5,17,1,1,1,13,4,
        3,0,11,6,1,2,16,1,2,1,10,1,3,6,7,13,0,
        0,8,4,3,5,16,1,1,2,5,4,7,4,15,0,5,0,6,
        3,6,5,17,2,0,1,17,1,0,2,8,3,6,3], 21, 4);
	# Store all responses in an array
	response = [response1, response2, response6];
	# Create n vectors
	ns1 = Vector{Int64}(undef, 21);
	ns2 = Vector{Int64}(undef, 21);
	ns6 = Vector{Int64}(undef, 21);
	for i in 1:21
		ns1[i] = sum(response1[i, :]);
		ns2[i] = sum(response2[i, :]);
		ns6[i] = sum(response6[i, :]);
	end;
	# Store all n vectors in an array
	ns = [ns1, ns2, ns6];
end;

# ╔═╡ fea8191e-65e8-4157-aea8-49050bcad41b
# Run the model for each response
begin
	chns_m14_2 = Vector{Chains}();
	for i in 1:3
		chns_i = sample(m14_2(response[i], ns[i]), MH(), MCMCThreads(), 50000, 4);
		push!(chns_m14_2, chns_i);
	end;
end;

# ╔═╡ b504986e-a36e-4d70-bc13-271f4550e1df
begin
	post_muc = Vector();
	post_mur = Vector();
	post_muu = Vector();
	for i in 1:3
		post_muci = Phi.(chns_m14_2[i]["muchat"][:]);
		post_muri = Phi.(chns_m14_2[i]["murhat"][:]);
		post_muui = Phi.(chns_m14_2[i]["muuhat"][:]);
		push!(post_muc, post_muci);
		push!(post_mur, post_muri);
		push!(post_muu, post_muui);
	end;
	muc_pdf = Vector();
	mur_pdf = Vector();
	muu_pdf = Vector();
	for i in 1:3
		muc_pdf_i = kde(post_muc[i]);
		mur_pdf_i = kde(post_mur[i]);
		muu_pdf_i = kde(post_muu[i]);
		push!(muc_pdf, muc_pdf_i);
		push!(mur_pdf, mur_pdf_i);
		push!(muu_pdf, muu_pdf_i);
	end;
end;

# ╔═╡ 3ee69022-e193-4cfa-9561-a759d5546c49
begin
	p12 = plot(x1, [pdf(muc_pdf[i], x1) for i in 1:3], 
		label = ["Trial 1" "Trial 2" "Trial 6"], 
		xlabel = "μ_c", ylabel = "Probability Density", 
		linewidth = 3, xlims = [0, 1])
	p22 = plot(x1, [pdf(mur_pdf[i], x1) for i in 1:3], 
		label = ["Trial 1" "Trial 2" "Trial 6"], 
		xlabel = "μ_r", ylabel = "Probability Density", 
		linewidth = 3, xlims = [0, 1])
	p32 = plot(x1, [pdf(muu_pdf[i], x1) for i in 1:3], 
		label = ["Trial 1" "Trial 2" "Trial 6"], 
		xlabel = "μ_u", ylabel = "Probability Density", 
		linewidth = 3, xlims = [0, 1])
	plot(p12, p22, p32, layout = (1, 3))
end
