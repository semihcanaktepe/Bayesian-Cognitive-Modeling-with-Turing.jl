...
# ╔═╡ 23500f76-33c0-11ee-29a9-2d15206e72bf
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 42f6f525-60c0-4c2a-a314-bf41bd31f547
md"# 5. Some examples of data analysis"

# ╔═╡ 5569234a-266d-4cd3-80aa-981ebb5b2924
md"## 5.1. Pearson correlation"

# ╔═╡ 9e524ef9-7a6f-4d9d-a6af-00f18ac8ec2f
@model function m5_1(x)
	mu1 ~ Normal(0, 0.001)
	mu2 ~ Normal(0, 0.001)
	...
	# Correlation Matrix
	T = [1 / lambda1 r * sigma1 * sigma2; r * sigma1 * sigma2 1 / lambda2]
	# add a small number for positive definiteness
	TI = Hermitian(inv(T + 1e-9 * I))
	for i in 1:size(x)[1]
		x[i, :] ~ MvNormal([mu1, mu2], TI)
	end;
end;

# ╔═╡ 53b35570-8d96-4171-bb20-4d608e7c680c
# Data
xs = [0.8 102; 1.0 98; 0.5 100; 0.9 105; 0.7 103; 
	0.4 110; 1.2 99; 1.4 87; 0.6 113; 1.1 89; 1.3 93];

# ╔═╡ 4936bc5d-4d1f-4ea9-83fa-3f01c56baa5b
# Standardized Data (returns a positive correlation)
begin
	xs2 = Matrix{Float64}(undef, 11, 2);
	xs2[:, 1] = ((xs[:, 1] .- mean(xs[:, 1])) ./ std(xs[:, 1]));
	xs2[:, 2] = ((xs[:, 2] .- mean(xs[:, 2])) ./ std(xs[:, 2]));
end;

# ╔═╡ f2ddcbd5-11db-4adb-aaa7-529ee87a7431
chns_m5_1 = sample(m5_1(xs), NUTS(), MCMCThreads(), 3000, 4);

# ╔═╡ 1bdd66c0-67b7-40bc-a6c8-2b6dd73beb29
describe(chns_m5_1)

# ╔═╡ d3b57812-1477-4499-b37a-5f62914682fb
plot(chns_m5_1)

# ╔═╡ 28a02183-babe-406f-92fa-992b338bbe0d
begin
	rt = xs[:, 1];
	iq = xs[:, 2];
	post_r = chns_m5_1["r"][:];
...
# ╔═╡ 7235b6ae-42e4-410d-81b8-ddc2c849cbc0
begin
	p1 = scatter(rt, iq, xlabel = "Response Time (sec)", ylabel = "IQ", 
		legend = false, xlims = [0, 1.5], ylim = [85, 115])
	begin
		p2 = density(post_r, xlabel = "Correlation", ylabel = "Posterior Density", 
		linewidth = 3, legend = false)
		vline!([mean_r], linewidth = 3)
	end
	plot(p1, p2, layout = (1, 2), size = (800, 400))
end

# ╔═╡ ae296a7f-a0db-4d75-8cc1-0f528eb9fe0a
md"## 5.2. Pearson correlation with uncertainty"

# ╔═╡ 0deefa3d-09d1-46c7-bf60-d453be372f24
chns_m5_2 = sample(m5_1(xs), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 674f2880-a4df-4b80-a93a-360e4dc314e9
describe(chns_m5_2)

# ╔═╡ 24aad491-43d7-4e10-9a42-3ff7947b0e97
plot(chns_m5_2)

# ╔═╡ 30268238-f106-4770-afdb-93fec21d9ae4
md"## 5.3. The kappa coefficient of agreement"

# ╔═╡ 1e4deba1-552e-4c6e-99c2-06d41b2494cb
@model function m5_3(y)
	alpha ~ Beta(1, 1)
	beta ~ Beta(1, 1)
	gamma ~ Beta(1, 1)
	pia = alpha * beta
	...
	pic = (1 - alpha) * (1 - gamma)
	pid = (1 - alpha) * gamma
	psi = (pia + pib) * (pia + pic) + (pib + pid) * (pic + pid)
	xi = alpha * beta + (1 - gamma) / gamma
	kappa = (xi - psi) / (1 - psi)
	y ~ Multinomial(sum(y), [pia, pib, pic, pid])
end;

# ╔═╡ e8d6e4cb-9141-450f-8fb3-d341129cbb4f
# Data
y = [14, 4, 5, 210];

# ╔═╡ 06bba665-9304-46ae-ad8b-e2b441f53d50
@model function m5_2(x)
	mu1 ~ Normal(0, 0.001)
	mu2 ~ Normal(0, 0.001)
	lambda1 ~ Gamma(0.001, 0.001)
	lambda2 ~ Gamma(0.001, 0.001)
	...
	# add a small number for positive definiteness
	TI = Hermitian(inv(T + 1e-9 * I))
	y ~ MvNormal([mu1, mu2], TI)
	x ~ Normal(y, lambdaerror)
end;

# ╔═╡ 60519382-24de-4c24-8f89-243d0abd4ef8
chns_m5_3 = sample(m5_3(y), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ f52cd0a1-d462-4048-b0ed-27e24f966363
describe(chns_m5_3)

# ╔═╡ 6f16b199-8d34-491b-a158-5caa0f9223c2
begin
	post_alpha = chns_m5_3["alpha"][:];
	post_beta = chns_m5_3["beta"][:];
	post_gamma = chns_m5_3["gamma"][:];
	pia = post_alpha .* post_beta;
	pib = post_alpha .* (1 .- post_beta);
	pic = (1 .- post_alpha) .* (1 .- post_gamma);
	pid = (1 .- post_alpha) .* post_gamma;
	psi = (pia .+ pib) .* (pia .+ pic) .+ (pib .+ pid) .* (pic .+ pid);
	xi = post_alpha .* post_beta .+ (1 .- post_gamma) ./ post_gamma;
	kappa = (xi .- psi) ./ (1 .- psi);
	mean_kappa = round(mean(kappa), digits = 3);
end;

# ╔═╡ 7b0c4cee-b0d4-4e85-91f0-c62e9081eb92
begin
	density(kappa, linewidth = 3, legend = false, title = "Posterior kappa", 
		xlabel = "Sample Value", ylabel = "Posterior Density")
	vline!([mean_kappa], linewidth = 3)
end

# ╔═╡ 3c5ed297-92e4-48bc-8d26-5dfe8d9d59a4
md"## 5.4. Change detection in time series data"

# ╔═╡ 132bb660-ef81-4c2c-aa02-a22495062144
@model function m5_4(c, t)
	#mu ~ filldist(Normal(0, 0.001), 2)
	#lambda ~ Gamma(0.001, 0.001)
	# Default priors did not work
	mu ~ arraydist([Normal(i, 5) for i in [40, 30]])
	lambda ~ Exponential(0.5)
	tau ~ DiscreteUniform(0, length(c))
	z = convert(Vector{Int64}, tau .>= t) .+ 1
	for i in 1:length(c)
		c[i] ~ Normal(mu[z[i]], lambda)
	end;
end;

# ╔═╡ 59f07797-92f5-4f78-bf3f-45422e1b5460
# Data
time = CSV.read("/Users/semih/Desktop/changepointdata.csv", DataFrame);

# ╔═╡ 8c442237-d175-403a-b4cb-be0aa68b6d9a
begin
	c = time.data;
	t = 1:length(c);
end;

# ╔═╡ f8e4800e-2545-4584-a442-359cdb93250f
chns_m5_4 = sample(m5_4(c, t), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ e974d3a1-3b48-4faa-8264-0514ed553b8d
# Model convergence takes ~55 seconds
# Sampling is so bad.
describe(chns_m5_4)

# ╔═╡ 66512a65-7c27-4c3e-b233-965af1f4ffc8
begin
	post_tau = chns_m5_4["tau"][:];
	post_mu1 = chns_m5_4["mu[1]"][:];
	post_mu2 = chns_m5_4["mu[2]"][:];
end;

# ╔═╡ 3745d26a-46f2-41b0-b4a4-da51a163ad07
begin
	m_tau = Int64(round(mean(post_tau), digits = 0));
	m_mu1 = mean(post_mu1);
	m_mu2 = mean(post_mu2);
	fitted = Vector{Float64}(undef, length(c));
	fitted[1:m_tau] = ones(m_tau)*m_mu1;
	fitted[m_tau:length(c)] = ones(length(c) - m_tau + 1)*m_mu2;
end;

# ╔═╡ 9c69605b-bf7b-40d0-9242-796a86a90546
begin
	plot(t, c, xlabel = "Time", ylabel = "Count", ylims = [10, 70],
		title = "Change in Time Series", legend = false,)
	plot!(fitted, linewidth = 3)
end

# ╔═╡ 27e1f51c-0ebc-4cdf-959d-6dbcc7cc22e7
md"## 5.5. Censored data"

# ╔═╡ 8a2093f2-c118-4bea-b705-4a515b4ca14c
@model function m5_5(z, y)
	theta ~ Uniform(0.25, 1)
	zlow = 15 .* (y .== 1) + 0 .* (y .== 0)
	zhigh = 25 .* (y .== 1) + 50 .* (y .== 0)
	for i in 1:length(z)
		z[i] ~ censored(Binomial(50, theta), zlow[i], zhigh[i])
	end;
end;

# ╔═╡ e5c37490-02db-44d8-9eb2-ec34f3ed8e48
# Data
begin
	# whether the value is observed or not
	obsy = vcat([1], zeros(949))
	# create a vector of missing values  
	missing_array = Vector{Union{Int, Missing}}(fill(missing, 949));
	# concatenate '30' at the end
	z = vcat(30, missing_array);
end;

# ╔═╡ 5c57ad8c-4b84-4dc9-9a82-5e5a0532bb39
# Does not sample or so slow
# chns_m5_5 = sample(m5_5(z, obsy), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ bb6aa874-0dc3-4503-b6e0-729984875c5d
md"## 5.6. Recapturing planes"

# ╔═╡ f347ef73-0372-41f9-88d1-5b2c4abb5a6f
@model function m5_6(x, k, n)
	tmax = 50
	t ~ DiscreteUniform(x + (n - k), tmax)
	#=
	# Zeros trick
	logterm1 = logfactorial(x) - logfactorial(k) - logfactorial(x - k)
  	logterm2 = logfactorial(t-x) - logfactorial(n-k) - logfactorial((t-x) - (n-k))
  	logterm3 = logfactorial(t) - logfactorial(n) - logfactorial(t - n)
  	C = 1000
...
# ╔═╡ ddc8743d-4fa2-4940-84b1-737cdc9086d6
# Data
x = 10; k = 4; n = 5;

# ╔═╡ 24748c31-25f4-4028-b91a-02aa133645e4
# NUTS does not work.
chns_m5_6 = sample(m5_6(x, k, n), MH(), MCMCThreads(), 1000, 4);

# ╔═╡ 5a40420d-8cae-4871-b9a4-4dd74dae3b3b
describe(chns_m5_6)

# ╔═╡ dcb46ee4-9816-4614-b342-e0f643090cdf
posterior_t = chns_m5_6["t"][:];

# ╔═╡ a54c7a75-fcdf-4a1c-8807-bb9de357b162
histogram(posterior_t, bins = 11:50, normalize=:pdf, title = "Posterior t",
	xlabel = "Number of Planes", ylabel = "Posterior Mass", legend = false,
	xlims = [10.5, 50.5], ylims = [0, 0.14])
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "543a6c0bdcbc1fdb572713114597df2241d3d024"
