...
# ╔═╡ 8003a666-375d-450d-aaab-b14100d2653d
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 9847689a-305c-11ee-0ae7-29c197068a81
md"# 3. Inferences with binomials"

# ╔═╡ 6c8a109e-6c81-4b4f-81b8-47d81d6a40fd
md"## 3.1. Inferring a rate"

# ╔═╡ ec59bea8-a7c4-4aeb-a2cd-0c38a2d4ceec
@model function m3_1(k, n)
    theta ~ Beta(1, 1)
    k ~ Binomial(n, theta)
end;

# ╔═╡ 42826987-592d-49d9-99db-bc3e0cb21439
# Data
n = 10; k = 5;

# ╔═╡ 613c6afb-24e4-4438-b3f3-f57d5444aa42
chns_m3_1 = sample(m3_1(k, n), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 4fc5ce1a-b907-4955-9bca-2d0ed779bd10
describe(chns_m3_1)

# ╔═╡ b60d705e-6060-451f-a773-376c6590f5dd
plot(chns_m3_1)

# ╔═╡ b005e371-2f10-4d60-be0a-296daee60f10
begin
	post_theta = chns_m3_1["theta"][:];
	mean_theta = round(mean(post_theta), digits = 3);
end;

# ╔═╡ 5fbc5a47-27b7-4e3d-9ddf-870dfa9dd3f9
begin
	density(post_theta, title = "Posterior theta", 
		legend = false, linewidth = 3, xlims = [0, 1])
	annotate!(mean_theta, 1.5, text(mean_theta, :center, 15))
	xlabel!("Rate")
	ylabel!("Posterior Density")
end

# ╔═╡ 21a96d35-80be-4513-96eb-ac856df2a3d5
md"## 3.2. Difference between two rates"

# ╔═╡ 28b02945-4dd2-4d5b-95eb-9c10beb26f15
@model function m3_2(k1, k2, n1, n2)
	theta1 ~ Beta(1, 1)
	theta2 ~ Beta(1, 1)
...
# ╔═╡ 151ee94a-62dc-448d-8760-8851ad7b4082
# Data
k1 = 5; k2 = 7; n1 = n2 = 10;

# ╔═╡ 858070fd-184a-48ae-a4b1-4eae667e5bbb
chns_m3_2 = sample(m3_2(k1, k2, n1, n2), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ d07ada47-9c55-4107-ac4f-b625ee579d44
describe(chns_m3_2)

# ╔═╡ 894d7ea2-8292-4188-9740-736be589bf0f
plot(chns_m3_2)

# ╔═╡ 185c800a-c0eb-4275-9f95-b997a266a808
begin
	delta = (chns_m3_2["theta1"] - chns_m3_2["theta2"])[:];
	mean_delta = round(mean(delta), digits = 3);
end;

# ╔═╡ 1b68aefd-d83b-4b52-b901-79897ea8143c
begin
	density(delta, title = "Posterior delta", 
		legend = false, linewidth = 3, xlims = [-1, 1])
	vline!([0], linewidth = 3)
	annotate!(mean_delta, 1.5, text(mean_delta, :center, 15))
	xlabel!("Differences in Rates")
	ylabel!("Posterior Density")
end

# ╔═╡ 02f96074-8f85-4d4a-9a0e-ae5a9367f4f1
md"## 3.3. Inferring a common rate"

# ╔═╡ 00cae291-df06-4819-a8e2-902531af6bb2
@model function m3_3(k, n)
	theta ~ Beta(1, 1)
	for i in 1:length(k)
...
# ╔═╡ 1c02ddbf-3f05-4175-a900-6028f6c03eef
begin
	# Data
	ks = [5, 7];
	ns = [10, 10]
...
# ╔═╡ c6595dcb-0de3-4a06-a8fd-7290f283c22f
chns_m3_3 = sample(m3_3(ks, ns), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 45867ea5-4cd5-42bb-a3ee-6c516163f2fb
describe(chns_m3_3)

# ╔═╡ 399bb68d-ad60-4e5e-91d4-7e8ad0d96ade
plot(chns_m3_3)

# ╔═╡ 5c3823c0-64aa-4ccb-9bad-b51724fedd4a
begin
	post_theta3 = chns_m3_3["theta"][:];
	mean_theta3 = round(mean(post_theta3), digits = 3);
end;

# ╔═╡ dad8208b-18ea-4572-9f38-19c51246412d
begin
	density(post_theta3, title = "Posterior theta", 
		legend = false, linewidth = 3)
	annotate!(mean_theta3, 1.5, text(mean_theta3, :center, 15), xlims = [0, 1])
	xlabel!("Rate")
	ylabel!("Posterior Density")
end

# ╔═╡ 0af6eafb-764d-46de-976a-109a20f20850
md"## 3.4. Prior and posterior distribution"

# ╔═╡ 8e9a8cd5-a6f8-436d-a187-aeeabd7595fb
@model function m3_4(k, n)
	theta ~ Beta(1, 1)
	k ~ Binomial(n, theta)
end;

# ╔═╡ fd58daa2-5519-48e2-bcb9-f041d826f376
# k = 1; n = 15
chns_m3_4 = sample(m3_4(1, 15), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 4061187a-4edf-42ab-9f7d-b9f962647004
describe(chns_m3_4)

# ╔═╡ cfb37fd7-b4f5-45c3-9432-bb19515fa3b3
plot(chns_m3_4)

# ╔═╡ 07c5948e-fb9a-45d3-9d9d-826dca13ad5c
# Plot of prior vs. posterior comparison
begin
	# Posterior theta and k
	post_theta4 = chns_m3_4["theta"][:]
	post_k = Vector{Int64}(undef, length(post_theta4));
	for i in 1:length(post_theta4)
		post_k[i] = rand(Binomial(15, post_theta4[i]))
	end;
	# Prior theta and k
	prior_theta = rand(Beta(1, 1), length(post_theta4))
	prior_k = Vector{Int64}(undef, length(post_theta4));
	for i in 1:length(post_theta4)
		prior_k[i] = rand(Binomial(15, prior_theta[i]))
	end;
end;

# ╔═╡ c338d79f-7067-44f2-b36c-d875040e32dd
begin
	# Density plot for theta
	density(prior_theta, linewidth = 3, linestyle = :dash,
		title = "Prior vs. Posterior theta", label = "Prior")
	density!(post_theta4, xlims = [0, 1], linewidth = 3,
		label = "Posterior")
	xlabel!("Rate")
	ylabel!("Density")	
end

# ╔═╡ 87b47401-8f81-453b-9715-e20e1468fb0d
begin
	histogram(prior_k, alpha = 0.75, label = "Prior", bins = 0:15,
		title = "Prior vs. Posterior k")
	histogram!(post_k, alpha = 0.75, label = "Posterior", bins = 0:15)
	xlabel!("Success Count")
	ylabel!("Count")
end

# ╔═╡ c17303b4-7e24-429c-9aee-093941c02476
md"## 3.5. Posterior prediction"

# ╔═╡ 514c6b19-c6c4-4bbf-9477-97bf46e7b78e
@model function m3_5(k, n)
	theta ~ Beta(1,1)
	#=
	for i in length(k)
		k[i] ~ Binomial(n[i], theta)
...
# ╔═╡ 94b431ab-3c1a-4bd9-a7db-751fdc247835
# ks = [0, 10]; ns = [10, 10]
chns_m3_5 = sample(m3_5([0, 10], [10, 10]), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ aedb7f89-d497-4cd3-9db7-d069f4f58e75
describe(chns_m3_5)

# ╔═╡ cf9abd58-a40d-45b9-a88a-26e15d16b02e
plot(chns_m3_5)

# ╔═╡ fc9c2c0c-484b-4e73-9ffb-63c2f5195ab1
begin
	post_theta5 = chns_m3_5["theta"][:];
	mean_theta5 = round(mean(post_theta5), digits = 3);
end;

# ╔═╡ e38a6313-401b-4beb-8336-9a2a74f3e290
begin
	density(post_theta5, title = "Posterior theta", 
		legend = false, linewidth = 3)
	annotate!(mean_theta5, 1.5, text(mean_theta5, :center, 15), xlims = [0, 1])
	xlabel!("Rate")
	ylabel!("Posterior Density")
end

# ╔═╡ 4620d2d8-2841-45e0-bd40-8236530df7a1
begin
	post_k1 = Vector{Int64}(undef, length(post_theta5));
	post_k2 = Vector{Int64}(undef, length(post_theta5));
	for i in 1:length(post_theta5)
		post_k1[i] = rand(Binomial(10, post_theta5[i]))
		post_k2[i] = rand(Binomial(10, post_theta5[i]))
	end;
end;

# ╔═╡ 0e1ef381-3888-4602-9a15-652ee01dbacc
begin
	marginalhist(post_k1, post_k2, 
		xlabel = "Success Count 1", 
		ylabel = "Success Count 2")
end

# ╔═╡ 1f65f6ec-5cb6-4bdf-bfdd-c6c1cb8a2693
md"## 3.6. Joint distributions"

# ╔═╡ d5c208a2-2412-4da1-a4df-39ac62315c00
@model function m3_6(k, nmax)
	theta ~ Beta(1, 1)
	p = fill(1 / nmax, nmax)
	n ~ Categorical(p)
	k .~ Binomial.(n, theta)
...
# ╔═╡ 9a5006b4-2557-4803-b77e-7d7333315160
# Data
begin
	nmax = 500;
	kvals = [16, 18, 22, 25, 27];
end;

# ╔═╡ acfd5c7d-c050-4757-b6e9-7e9e6c250e44
# NUTS sampler does not work!
chns_m3_6 = sample(m3_6(kvals, nmax), MH(), MCMCThreads(), 3000, 4);

# ╔═╡ 41b90c23-f6ea-430a-98c6-8c37a1467881
describe(chns_m3_6)

# ╔═╡ cb72c188-e13c-4081-92ad-ffda6c685e18
plot(chns_m3_6)

# ╔═╡ c5388cef-83f0-4283-a2d5-3ff5e0e62a92
begin
	post_theta6 = chns_m3_6["theta"][:];
	post_n = chns_m3_6["n"][:];
end;
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "08ea1c30ac41ea05a18b47101e8d3e6fbb92c5fe"
