...
# ╔═╡ d700accd-5071-4018-82b9-984141cba28e
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ eb210e5a-309c-11ee-2008-ddc186eae9a3
# 5. Some examples of data analysis

# ╔═╡ d7d7645a-a55f-4a38-b857-e531debf04a4
# 5.1. Pearson correlation

# ╔═╡ 356b5b79-927d-4cb4-b7e8-9a543d5632ea
@model function m5_1(x)
	mu ~ filldist(Normal(0, 0.001), 2)
	#mu1 ~ Normal(0, 0.001)
	#mu2 ~ Normal(0, 0.001)
	#mu ~ MvNormal([0.0, 0.0], [0.001 0.0; 0.0 0.001])
...
# ╔═╡ acd62f50-fda0-4a14-b68d-d915e72d7bb1
# Data
xs = reshape([0.8, 102, 1.0, 98, 0.5, 100, 0.9, 105, 0.7, 103, 0.4,
	110, 1.2, 99, 1.4, 87, 0.6, 113, 1.1, 89, 1.3, 93], 11, 2);

# ╔═╡ 14740788-f7e2-45fb-a8c0-c3159afaeb2f
chns_m5_1 = sample(m5_1(xs), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ f879ad8a-b74d-4f1d-b404-94e53522730c
describe(chns_m5_1)

# ╔═╡ 98f597a4-c70e-4e8f-92fb-67a986517d7b
# 5.2. Pearson correlation with uncertainty

# ╔═╡ 11c16fdc-fce1-4697-b27c-4e8f35767d3d
...
# ╔═╡ 1c88ac00-d28c-4831-ba94-65d87e621796
chns_m5_2 = sample(m5_2(xs), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 00efae56-ab8a-4dc7-b557-0b5770919f65
describe(chns_m5_2)

# ╔═╡ 12f125db-6f8d-45f1-92d6-b3efccc7db35
# 5.3. The kappa coefficient of agreement

# ╔═╡ 8f53a243-2203-4d23-8f3b-7c2b03ed3361
@model function m5_3(y)
	...
	epsilon = alpha * beta + (1 - gamma) / gamma
	kappa = (epsilon - psi) / (1 - psi)
	y .~ Multinomial(sum(y), [pia, pib, pic, pid])
end;

# ╔═╡ 78427ac8-8dd6-4a56-ae14-e568370d1d55
# ╠═╡ disabled = true
#=╠═╡
# Data
y = [14, 4, 5, 210];
...
# ╔═╡ 516ba1d0-4189-402e-bad9-7ccc0b7df356
#=╠═╡
chns_m5_3 = sample(m5_3(y), NUTS(), MCMCThreads(), 1000, 4);
  ╠═╡ =#

# ╔═╡ bec0bd04-e080-45e5-9ca2-f3c639084d22
#=╠═╡
describe(chns_m5_3)
  ╠═╡ =#

# ╔═╡ 06c48a68-78ce-4f51-a2ba-d8070fb808c6
#=╠═╡
plot(chns_m5_3)
  ╠═╡ =#

# ╔═╡ e12b69d5-e9c5-45e1-b97b-8a90fea4e2c5
#=╠═╡
# Calculate the deterministic parameters
begin
	pia = chns_m5_3["alpha"] .* chns_m5_3["beta"];
	pib = chns_m5_3["alpha"] .* (1 .- chns_m5_3["beta"]);
	pic = (1 .- chns_m5_3["alpha"]) .* (1 .- chns_m5_3["gamma"]);
	pid = (1 .- chns_m5_3["alpha"]) .* chns_m5_3["gamma"];
	psi = (pia .+ pib) .* (pia .+ pic) .+ (pib .+ pid) .* (pic .+ pid);
	epsilon = chns_m5_3["alpha"] .* chns_m5_3["beta"] .+ (1 .- chns_m5_3["gamma"]) ./ 	chns_m5_3["gamma"];
	kappa = (epsilon .- psi) ./ (1 .- psi);
end;
  ╠═╡ =#

# ╔═╡ d6c95439-a58e-49a8-a795-80366486ebc4
#=╠═╡
density(pia, title = "pia")
  ╠═╡ =#

# ╔═╡ 8c209d29-119b-469e-a5f2-0dd012e2bccb
#=╠═╡
density(pib, title = "pib")
  ╠═╡ =#

# ╔═╡ 36af5cd1-db6a-49be-8840-964cf3d15ac1
#=╠═╡
density(pic, title = "pic")
  ╠═╡ =#

# ╔═╡ 472d5399-9cb9-4b66-9e0a-e660224434a3
#=╠═╡
density(pid, title = "pid")
  ╠═╡ =#

# ╔═╡ a99b1c6b-6177-44ce-bb0c-d2b08b65451b
#=╠═╡
density(psi, title = "psi")
  ╠═╡ =#

# ╔═╡ d67eda97-66ab-4adf-be95-7a3b88b2be87
#=╠═╡
density(epsilon, title = "epsilon")
  ╠═╡ =#

# ╔═╡ d22d15b0-0cb6-4bad-b8b2-237d3b30fcb8
#=╠═╡
density(kappa, title = "kappa")
  ╠═╡ =#

# ╔═╡ bddbbf88-4b48-4fe5-8869-c4f610fe90de
# 5.4. Change detection in time series data

# ╔═╡ a8f52ac6-9802-42b8-9733-05baff9ec399
@model function m5_4(c, series)
	mu1 ~ Normal(0, 0.001)
	mu2 ~ Normal(0, 0.001)
	lambda ~ Gamma(0.001, 0.001)
	tau ~ DiscreteUniform(0, length(c))
	mu = @. ifelse(tau >= series, mu1, mu2)
	c .~ Normal.(mu, lambda)
end;

# ╔═╡ 8c730975-86e9-40e9-87c2-3176a5973b79
# Data
time = CSV.read("/Users/semih/Desktop/changepointdata.csv", DataFrame);

# ╔═╡ 15eaae33-811c-47bd-bdd5-42b999267b25
begin
	c = time.data;
	series = 1:length(c);
end;

# ╔═╡ 54123404-7e28-4d54-a196-b387b4734cae
chns_m5_4 = sample(m5_4(c, series), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 9c661209-1552-4d30-94eb-c54a593053df
describe(chns_m5_4)

# ╔═╡ fdc5a6dc-ca67-4f68-94f4-e55f7b61b24f
# 5.5. Censored data

# ╔═╡ b630cc69-be3d-46c3-a691-9a6aefb5b812
@model function m5_5(z, y)
	...
	#z .~ censored.(Binomial(50, theta), zlow, zhigh)
end;

# ╔═╡ 889984a8-fc4f-40dc-9add-9de6c5539175
# Data
begin
...
# ╔═╡ 1a78230c-e1ec-4328-80f6-d6f0a48d5e17
chns_m5_5 = sample(m5_5(z, obsy), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 9e600228-ad4c-45ad-a92b-0582b3bd5f32
# 5.6. Recapturing planes

# ╔═╡ 6486efc3-3126-4fae-a64e-f41eb0475a45
@model function m5_6(x, k, n)
	t ~ DiscreteUniform((x - (n-k)), 50)
...
# ╔═╡ 83e2819c-016b-475b-8e9f-ba0ead06a4a3
# Data
x = 10; k = 4; n = 5;

# ╔═╡ d1ea1e0a-805a-40b6-a908-fbb4326ccdbc
chns_m5_6 = sample(m5_6(x, k, n), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 17f0ec5a-ceae-4e65-81c5-32cbd092763c
describe(chns_m5_6)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "8c500af3e1fcfe2e6020def567aab50d8a1e2992"
