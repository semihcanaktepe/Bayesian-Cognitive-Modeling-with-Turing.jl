...
# ╔═╡ 84ea1881-24ab-4a4a-9517-1af57651144b
begin
	using Turing; 
	using Distributions;
	using StatsPlots;
...
# ╔═╡ f0b17c90-3094-11ee-1a64-99a52a5ff515
# 4. Inferences with Gaussians

# ╔═╡ a5b0a16b-4d93-42b6-b51c-6335c76406a1
# 4.1 Inferring a mean and standard deviation

# ╔═╡ 86de3b8e-7251-4e83-baf2-186c67befffe
@model function m4_1(x)
	mu ~ Normal(0, 0.001)
	sigma ~ Uniform(0, 10)
	x .~ Normal.(mu, 1/sigma^2)
end;

# ╔═╡ 45587f07-9923-45c5-928c-5d1d10833435
# Data
x = [1.1, 1.9, 2.3, 1.8];

# ╔═╡ 3a96a0c3-5a9c-4579-b6ee-ece49d2180e3
chns_m4_1 = sample(m4_1(x), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 5f425314-c408-46df-8e23-01f03584230c
describe(chns_m4_1)

# ╔═╡ ffcc9bae-084b-4179-9183-9d716b007bb7
plot(chns_m4_1)

# ╔═╡ f6602275-b84d-464a-8982-a94ef5e56412
# The seven scientists

# ╔═╡ 69a605c1-63a9-4854-88d8-83d84f1cf485
# Data
xs = [-27.020, 3.570, 8.191, 9.898, 9.603, 9.945, 10.056];

# ╔═╡ 5ffb67ff-879a-4893-9974-a9e42913ee7e
# 4.3. Repeated measurement of IQ

# ╔═╡ 06d10518-d9ba-4266-8acc-3a72a5eb956d
@model function m4_3(x)
	mu ~ filldist(Uniform(0,300), 3)
	sigma ~ Uniform(0, 100)
	lambda = 1/sigma^2
	for i in 1:3
		for j in 1:3
			x[i][j] ~ Normal(mu[i], lambda)
		end;
	end;
end;

# ╔═╡ bbfca32d-50d5-44ff-86ca-2d4bf202fa5a
# Data
iqs = [[90, 95, 100], [105, 110, 115], [150, 155, 160]];

# ╔═╡ 19c8b7e7-905b-4653-95c9-24f52305924a
chns_m4_3 = sample(m4_3(iqs), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ b9e1b8bc-fe3e-46ec-a655-927b3e9d1043
describe(chns_m4_3)

# ╔═╡ aa2925fd-27ed-468e-a423-f646d67ffebb
plot(chns_m4_3)

# ╔═╡ 9ade22e7-afd6-427d-99a4-08db020d17e1
# Calculate lambda
lambda = 1 ./ chns_m4_3["sigma"] .^ 2;

# ╔═╡ e2d898f6-baf3-4afa-a895-95fed437a9db
@model function m4_2(x)
	#= Diagnostic values are so bad with the default priors,
	so less informative priors are used for lambda. On top of that,
	prior for mu is not fine given the x values.
	=#
	#mu ~ Normal(0, 0.001)
	mu ~ Normal(9, 1)
	#lambda ~ filldist(Gamma(0.001, 0.001), length(x))
	lambda ~ filldist(Gamma(0.1, 0.1), length(x))
	#= 
	for i in 1:length(x)
...
# ╔═╡ ec8e07e9-ef7a-4344-9255-3bea9cc19852
chns_m4_2 = sample(m4_2(xs), NUTS(), MCMCThreads(), 2000, burnin = 100, 4);

# ╔═╡ eb88f17b-1beb-409d-9ce7-fa1ad12f923e
describe(chns_m4_2)

# ╔═╡ a0d10289-a0cb-4a2c-8510-0fbd5fce56d1
plot(chns_m4_2)

# ╔═╡ 0dd44888-17de-451b-868b-47c590a5af8d
plot(lambda, title = "lambda")

# ╔═╡ e9b39549-0215-4676-91a7-77b225b6dc28
density(lambda, title = "lambda")
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "085d9e3c4b5f251885bed40f43324f660e4a0603"
