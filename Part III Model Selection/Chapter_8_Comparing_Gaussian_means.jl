...
# ╔═╡ c03ab430-9675-4376-9ee9-026c093d750a
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ 9d5f6d00-3601-11ee-26e3-1f432c49d662
md"# 8. Comparing Gaussian means"

# ╔═╡ 8f21aa7c-2c3b-49d0-8985-b5d71e23a17c
md"## 8.1. One-sample comparison"

# ╔═╡ a00a9978-86b9-4825-9d13-65e2e4042263
@model function m8_1(x)
	delta ~ Cauchy(0, 1)
	sigma ~ truncated(Cauchy(0, 1), 0, Inf)
	mu = delta * sigma
	x .~ Normal.(mu, 1 / sigma^2)
end;

# ╔═╡ 99bd2f02-60bb-481b-9f3f-b64dea9c2f92
# Data
begin
	winter = [-0.05,0.41,0.17,-0.13,0.00,-0.05,0.00,0.17,0.29,0.04,0.21,0.08,0.37,
            ...
            0.04,0.25,0.12];
	summer = [0.00,0.38,-0.12,0.12,0.25,0.12,0.13,0.37,0.00,0.50,0.00,0.00,-0.13,
...
# ╔═╡ 82497d94-1931-408a-b24e-6465bf6ea713
chns_m8_1 = sample(m8_1(x1), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 1a17a0e9-1c6b-40bc-a965-29d5f73b2f28
describe(chns_m8_1)

# ╔═╡ 44bd2a7a-7754-44c4-877b-d4723d672434
plot(chns_m8_1)

# ╔═╡ 84366499-9788-42aa-b17d-087521aea87a
begin
	post_delta = chns_m8_1["delta"][:];
...
# ╔═╡ a55a3f5a-1745-470d-bbc4-63bf43340321
begin
...
# ╔═╡ 03e2a1f6-4357-4e1e-98ea-c62bbabe606e
md"## 8.2. Order-restricted one-sample comparison"

# ╔═╡ 7b95415b-951a-4880-ab97-0093fbafb387
@model function m8_2(x)
	delta ~ truncated(Cauchy(0, 1), -Inf, 0)
	sigma ~ truncated(Cauchy(0, 1), 0, Inf)
	mu = delta * sigma
	x .~ Normal.(mu, 1 / sigma^2)
end;

# ╔═╡ 13f7f5df-d44b-4693-b145-f6039866c8d9
chns_m8_2 = sample(m8_2(x1), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ fbd83f24-e109-4664-b179-b9f21d8e27e0
describe(chns_m8_2)

# ╔═╡ 98416169-22a7-417e-9c19-66899eb334ab
plot(chns_m8_2)

# ╔═╡ 0304ff68-eba2-4674-ad49-c8d44da58b37
begin
	post_delta2 = chns_m8_2["delta"][:];
	delta_pdf2 = kde(post_delta2);
	x2 = range(-3, 0, length = 100)
	prior_pdf2 = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf2[i] = pdf(truncated(Cauchy(0, 1), -Inf, 0), x2[i])
	end;
	posterior2 = pdf(delta_pdf2, 0);
	prior2 = pdf(truncated(Cauchy(0, 1), -Inf, 0), 0);
	BF01_2 = posterior2 / prior2;
end;

# ╔═╡ 13933872-f3ce-470a-bf13-a241ecb37418
begin
...
# ╔═╡ c9c2deac-cb29-4cee-85fb-953c587b898d
md"## 8.3. Two-sample comparison"

# ╔═╡ 543fb968-df8d-4330-a831-215029bee396
@model function m8_3(x, y)
	delta ~ Cauchy(0, 1)
	mu ~ Cauchy(0, 1)
	sigma ~ truncated(Cauchy(0, 1), 0, Inf)
	alpha = delta * sigma
	mux = mu + alpha / 2
	muy = mu - alpha / 2
	x .~ Normal.(mux, 1 / sigma^2)
	y .~ Normal.(muy, 1 / sigma^2)
end;

# ╔═╡ f56d1e96-54b9-4005-98fa-e751a86e1b66
# Data
begin
	xdat = [70,80,79,83,77,75,84,78,75,75,78,82,74,81,72,70,75,72,76,77];
	ydat = [56,80,63,62,67,71,68,76,79,67,76,74,67,70,62,65,72,72,69,71];
	# Standardize
	xdata = (xdat .- mean(xdat)) / std(xdat);
	ydata = (ydat .- mean(ydat)) / std(ydat);
end;

# ╔═╡ f2f9a3a3-83ac-4b87-ab20-1e199673b94e
chns_m8_3 = sample(m8_3(xdata, ydata), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 51fe9927-7f36-414a-80c9-a04194fdcefe
describe(chns_m8_3)

# ╔═╡ debeb74c-4df0-4ea3-b88d-f0c8ba71e2ab
plot(chns_m8_3)

# ╔═╡ 9071e82e-bf22-4de7-b617-563c3f80b56d
begin
	post_delta3 = chns_m8_3["delta"][:];
	delta_pdf3 = kde(post_delta3);
	prior_pdf3 = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf3[i] = pdf(Cauchy(0, 1), x[i])
	end;
	posterior3 = pdf(delta_pdf3, 0);
	prior3 = pdf(Cauchy(0, 1), 0);
	BF01_3 = posterior3 / prior3;
end;

# ╔═╡ ec7aa5c5-af1a-4f53-80ef-e18e44f97c9d
begin
	plot(x, pdf(delta_pdf3, x), title = "Prior vs. Posterior delta", 
		xlabel = "Delta", ylabel = "Density", label = "Posterior",
		linestyle = :dash, linewidth = 3)
	plot!(x, prior_pdf3, linewidth = 3, label = "Prior")
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "8dc2c85eb9f10a90ef403ecdacd29ba0eb8aca5c"
