...
# ╔═╡ 00a08566-a034-4659-9053-5d3dc746351b
begin
	using Turing;
	using Distributions;
	using StatsPlots;
...
# ╔═╡ e3d50a82-3611-11ee-3536-b75c2841a2b9
md"# 9. Comparing binomial rates"

# ╔═╡ 23aa5970-60ea-4e6e-80eb-e842277d2ac1
md"## 9.1. Equality of proportions"

# ╔═╡ 51f63d1e-9673-44e8-a010-2e04b866aa62
@model function m9_1(s1, s2, n1, n2)
	theta1 ~ Beta(1, 1)
	theta2 ~ Beta(1, 1)
	delta = theta1 - theta2
...
# ╔═╡ 4660bccc-aae3-4926-8de3-96335b989de7
# Data
s1 = 424; s2 = 5416; n1 = 777; n2 = 9072;

# ╔═╡ 61b16c9a-37e9-42c6-b8da-dd07f66a9778
chns_m9_1 = sample(m9_1(s1, s2, n1, n2), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ beba49e9-6a1b-4a1c-bb92-0aa2a10af336
describe(chns_m9_1)

# ╔═╡ 7f79c100-7893-4ff6-baf3-003f443c76ed
plot(chns_m9_1)

# ╔═╡ c624a9cb-9d09-4528-9006-3183b7560a0b
begin
	post_theta1 = chns_m9_1["theta1"][:];
	post_theta2 = chns_m9_1["theta2"][:];
	post_delta = post_theta1 - post_theta2;
end;

# ╔═╡ 9facda66-71e2-46a4-a1e2-331f4ba1a480
begin
	delta_pdf = kde(post_delta);
	x = range(-3, 3, length = 100)
	prior_pdf = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf[i] = pdf(Cauchy(0, 1), x[i])
	end;
	posterior = pdf(delta_pdf, 0);
	prior = pdf(Cauchy(0, 1), 0);
	BF01 = posterior / prior;
end;

# ╔═╡ d7dbc427-8ce0-471b-b8d5-93357c50c6ae
begin
	plot(x, pdf(delta_pdf, x), title = "Prior vs. Posterior delta", 
		xlabel = "Delta", ylabel = "Density", label = "Posterior",
		linestyle = :dash, linewidth = 3)
	plot!(x, prior_pdf, linewidth = 3, label = "Prior")
	plot!([0, 0], [posterior, prior], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior, prior], color = "black", label = "")
end

# ╔═╡ 6b38657f-0645-440b-ad01-54e74b7a3119
md"## 9.2. Order-restricted equality of proportions"

# ╔═╡ 04db2ecf-bda7-43ce-ad3b-7df5e7fdc4e2
# Define Phi function
function Phi(x::Real)
    return 0.5 + 0.5 * erf(x / sqrt(2))
end;

# ╔═╡ c7c0937a-8db7-415a-b6af-52c6db3bc740
@model function m9_2(s1, s2, n1, n2)
	angle = 45 * 3.1416 / 180
	mu1 = 0
  	mu2 = 0
	TI = [1 0; 0 1]
	thetap ~ MvNormal([mu1, mu2], TI)
...
# ╔═╡ 867ac842-bae1-4738-9afc-529516aa62b9
chns_m9_2 = sample(m9_2(s1, s2, n1, n2), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ 550ef68b-bad8-436c-ae57-af6192a7bf6f
describe(chns_m9_2)

# ╔═╡ 90ea3534-418b-442f-897c-9f5b01049a9e
plot(chns_m9_2)

# ╔═╡ 8b4346fc-3b9b-4a43-930a-41fd23b3987f
begin
	post_theta1_2 = chns_m9_2["thetap[1]"][:];
	post_theta2_2 = chns_m9_2["thetap[2]"][:];
	post_delta2 = post_theta1_2 - post_theta2_2;
end;

# ╔═╡ cd550062-767e-4573-a372-fd98dc34f43a
begin
	delta_pdf2 = kde(post_delta2);
	x2 = range(-1, 0, length = 100)
	prior_pdf2 = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf2[i] = pdf(Cauchy(0, 1), x2[i])
	end;
	posterior2 = pdf(delta_pdf2, 0);
	prior2 = pdf(Cauchy(0, 1), 0);
	BF01_2 = posterior2 / prior2;
end;

# ╔═╡ 62774ce6-8ddb-41e5-8034-48c453a94efc
begin
	plot(x2, pdf(delta_pdf2, x2), title = "Prior vs. Posterior delta", 
		xlabel = "Delta", ylabel = "Density", label = "Posterior",
		linestyle = :dash, linewidth = 3)
	plot!(x2, prior_pdf2, linewidth = 3, label = "Prior")
	plot!([0, 0], [posterior2, prior2], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior2, prior2], color = "black", label = "")
end

# ╔═╡ d40c6549-0f21-41fa-ace7-8e8fe0e11c3f
md"## 9.3. Comparing within-subject proportions"

# ╔═╡ a6c5f6bd-deef-4fd2-8f7a-ee9383dd4c27
@model function m9_3(sn, sb, nn, nb)
	mu ~ truncated(Normal(0, 1), 0, Inf)
	sigma ~ Uniform(0, 10)
	delta ~ truncated(Normal(0, 1), 0, Inf)
	sigma_a ~ Uniform(0, 10)
	mua = delta * sigma_a
	alpha ~ Normal(mua, 1 / sigma_a^2)
	phin ~ Normal(mua, 1 / sigma^2)
	phib = phin .+ alpha
	thetan = Phi.(phin)
	thetab = Phi.(phib)
...
# ╔═╡ c9160cdc-527c-4832-9335-53affb86636a
# Data
begin
	sb = [15,11,15,14,15,18,16,16,18,16,15,13,18,12,11,13,17,18,16,11,17,18,
	...
	sn = [15,12,14,15,13,14,10,17,13,16,16,10,15,15,10,14,17,18,19,12,19,18,
...
# ╔═╡ 28dcaaa5-f34c-45d8-a33a-c02f7b47d2aa
chns_m9_3 = sample(m9_3(sn, sb, nn, ns), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ e0076f4a-0125-4659-9d03-76d0b4fc0a76
describe(chns_m9_3)

# ╔═╡ e4a77a92-2b28-411d-9b29-8032bae38370
plot(chns_m9_3)

# ╔═╡ 450f1dbf-114e-4031-aff7-f7837993cfab
begin
	post_delta3 = chns_m9_3["delta"][:];
	delta_pdf3 = kde(post_delta3);
	x3 = range(0, 4, length = 100)
	prior_pdf3 = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf3[i] = pdf(Normal(0, 1), x3[i])
	end;
	posterior3 = pdf(delta_pdf3, 0);
	prior3 = pdf(Normal(0, 1), 0);
	BF01_3 = posterior3 / prior3;
end;

# ╔═╡ 1446e724-3907-4993-b031-f0e96536e4e9
begin
	plot(x3, pdf(delta_pdf3, x3), title = "Prior vs. Posterior delta", 
		xlabel = "Delta", ylabel = "Density", label = "Posterior",
		linestyle = :dash, linewidth = 3)
	plot!(x3, prior_pdf3, linewidth = 3, label = "Prior")
	plot!([0, 0], [posterior3, prior3], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior3, prior3], color = "black", label = "")
end

# ╔═╡ ee0c249b-099c-4e36-8b9e-2cceb877cefc
md"## 9.4. Comparing between-subject proportions"

# ╔═╡ 10e1e129-a93b-4bb0-9fc9-50d0ccf20566
@model function m9_4(sc, sa, nc, na)
	delta ~ Normal(0, 1)
	mu ~ Normal(0, 1)
	sigma ~ Uniform(0, 10)
	alpha = delta * sigma
	muc = mu + alpha / 2
	mua = mu - alpha / 2
	phic ~ Normal(muc, 1 / sigma^2)
	phia ~ Normal(mua, 1 / sigma^2)
	thetac = Phi.(phic)
	thetaa = Phi.(phia)
	sc .~ Binomial.(nc, thetac)
	sa .~ Binomial.(na, thetaa)
end;

# ╔═╡ f3e0d49b-d638-4bc5-b35e-ae3bec1b5422
# Data
begin
	# Controls
	numerrors1 = [15,10,61,11,60,44,63,
		70,57,11,67,21,89,12,63,11,
		96,10,37,19,44,18,78,27,60,14]
	nc = [89,74,128,87,128,121,128,128,128,78,128,
		106,128,83,128,100,128,73,128,86,128,86,128,100,128,79];
	kc = nc - numerrors1;
	nsc = length(kc);
	# ADHD
	numerrors2 = [88,50,58,17,40,18,21,50,21,69,19,29,11,76,46,
		36,37,72,27,92,13,39,53,31,49,57,17,10,12,21,39,43,49,
		17,39,13,68,24,21,27,48,54,41,75,38,76,21,41,61,24,28,21]
	na = [128,128,128,86,128,117,89,128,110,128,93,107,87,128,128,
		113,128,128,98,128,93,116,128,116,128,128,93,86,86,96,128,
		128,128,86,128,78,128,111,100,95,128,128,128,128,128,128,98,
		127,128,93,110,96];
	ka = na - numerrors2;
	nsa = length(ka);
end;

# ╔═╡ 2640638a-6f66-4772-9095-8aeb7b3b1fb3
chns_m9_4 = sample(m9_4(kc, ka, nc, na), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ f836d6cb-97c9-4936-aced-e56cdcdca182
describe(chns_m9_4)

# ╔═╡ 227fbef0-21b2-49ea-99da-edfa438b76c0
plot(chns_m9_4)

# ╔═╡ aec71a68-7139-44fe-ba36-acbadba42663
begin
	post_delta4 = chns_m9_4["delta"][:];
	delta_pdf4 = kde(post_delta4);
	prior_pdf4 = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf4[i] = pdf(Normal(0, 1), x[i])
	end;
	posterior4 = pdf(delta_pdf4, 0);
	prior4 = pdf(Normal(0, 1), 0);
	BF01_4 = posterior4 / prior4;
end;

# ╔═╡ 7fb4cc48-ed16-44ba-94c1-d4dab6e51818
begin
	plot(x, pdf(delta_pdf4, x), title = "Prior vs. Posterior delta", 
		xlabel = "Delta", ylabel = "Density", label = "Posterior",
		linestyle = :dash, linewidth = 3)
	plot!(x, prior_pdf4, linewidth = 3, label = "Prior")
	plot!([0, 0], [posterior4, prior4], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior4, prior4], color = "black", label = "")
end

# ╔═╡ 3fd8f189-1ba5-42a6-a4e2-dea59b30b887
md"## 9.5. Order-restricted between-subjects comparison"

# ╔═╡ e16d39d7-3fb7-41ad-987d-372c25e85f8f
@model function m9_5(sc, sa, nc, na)
	delta ~ truncated(Normal(0, 1), 0, Inf)
	mu ~ Normal(0, 1)
	sigma ~ Uniform(0, 10)
	alpha = delta * sigma
...
# ╔═╡ 621b941b-10e0-4456-96da-0d536e18ff3a
chns_m9_5 = sample(m9_5(kc, ka, nc, na), NUTS(), MCMCThreads(), 1000, 4);

# ╔═╡ ed438731-f640-417d-9524-73e173b3d485
describe(chns_m9_5)

# ╔═╡ 7ab1408f-1eb8-410c-9344-0c08477a3bf3
plot(chns_m9_5)

# ╔═╡ 068aabce-c212-448f-94b3-031eb163d719
begin
	post_delta5 = chns_m9_5["delta"][:];
	delta_pdf5 = kde(post_delta5);
	x5 = range(0, 3, length = 100)
	prior_pdf5 = Vector{Float64}(undef, 100);
	for i in 1:100
		prior_pdf5[i] = pdf(Normal(0, 1), x5[i])
	end;
	posterior5 = pdf(delta_pdf5, 0);
	prior5 = pdf(Normal(0, 1), 0);
	BF01_5 = posterior5 / prior5;
end;

# ╔═╡ 883318bf-b3ef-48fe-babb-a321fb1f6eb5
begin
	plot(x5, pdf(delta_pdf5, x5), title = "Prior vs. Posterior delta", 
		xlabel = "Delta", ylabel = "Density", label = "Posterior",
		linestyle = :dash, linewidth = 3)
	plot!(x5, prior_pdf5, linewidth = 3, label = "Prior")
	plot!([0, 0], [posterior5, prior5], color = :black, linewidth = 2, label = "")
	scatter!([0, 0], [posterior5, prior5], color = "black", label = "")
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "34e5123342007eb0b09a0acc1a3b8c628967a3d9"
